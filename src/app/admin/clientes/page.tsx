...
const isMerchandisingVisit = (visita: any) => {
  ...
  return (
    tv === "merchandising" ||
    tv === "qualid-merchandising" ||
    tv === "shell-merchandising" ||
    tv === "signage-capture" ||
...
export default function GestionClientesPage() {
  ...
  const [selectedVisitType, setSelectedVisitType] = useState<
    "Merchandising" | "Trade (Eventos)" | "Trade (Impulso)" | "sin_configurar"
  >("sin_configurar");
  // Búsqueda y filtros
  ...
  const [filterTipo, setFilterTipo] = useState<
    "todos" | "tienda" | "distribuidor" | "cliente_especial"
  >("todos");
  const [filterSinVisita, setFilterSinVisita] = useState<
    "todos" | "7" | "15" | "30" | "60" | "90"
  >("todos");
  const [filterSeñalizacion, setFilterSeñalizacion] = useState<
    "todos" | "con_señalizacion" | "sin_señalizacion" | "sin_informacion"
  >("todos");
      ...
      try {
        ...
        if (result) {
          ...
          if (!result.permissions.canManageClients) {
            console.log(
              "⚠️ Sin permisos para gestionar clientes, redirigiendo..."
            );
  ...
  useEffect(() => {
    ...
    if (
      currentUser &&
      currentUser.sede &&
      formData.sede === "GRUPO DISBATTERY"
    ) {
      console.log(
        "📍 Inicializando formulario con sede del usuario:",
        currentUser.sede
      );
      setFormData((prev) => ({
        ...prev,
   ...
   */
  const obtenerUltimasVisitasPorTipo = async (
    rifCliente: string
  ): Promise<{
    ...
    try {
      console.log(
        `🔍 Buscando últimas visitas por tipo para cliente RIF: ${rifCliente}`
      );
      ...
      if (visitasSnapshot.empty) {
        console.log(
          `⚠️ No se encontraron visitas para el cliente ${rifCliente}`
        );
        return {
      ...
      console.log(
        `🔍 DEBUG VISITAS - Cliente ${rifCliente}:`,
        visitas.map((v: any) => ({
          id: v.id,
      ...
      console.log(
        `   - Merchandising encontradas: ${visitasMerchandising.length}`
      );
      console.log(
        `   - Trade Impulso encontradas: ${visitasTradeImpulso.length}`
      );
      if (visitasMerchandising.length > 0) {
        console.log(
          `   - Última Merchandising: ${(visitasMerchandising[0] as any).createdAt?.toDate ? (visitasMerchandising[0] as any).createdAt.toDate().toISOString() : (visitasMerchandising[0] as any).createdAt}`
        );
      }
      if (visitasTradeImpulso.length > 0) {
        console.log(
          `   - Última Trade Impulso: ${(visitasTradeImpulso[0] as any).createdAt?.toDate ? (visitasTradeImpulso[0] as any).createdAt.toDate().toISOString() : (visitasTradeImpulso[0] as any).createdAt}`
        );
      }

      const resultado = {
        ultimaVisitaMerchandising:
          visitasMerchandising.length > 0
            ? getDateFromCreatedAt(visitasMerchandising[0])
            : null,
        ultimaVisitaTradeImpulso:
          visitasTradeImpulso.length > 0
            ? getDateFromCreatedAt(visitasTradeImpulso[0])
            : null,
      };

      console.log(`📊 Últimas visitas por tipo para cliente ${rifCliente}:`, {
        merchandising: resultado.ultimaVisitaMerchandising
          ? resultado.ultimaVisitaMerchandising.toLocaleDateString()
          : "Sin visitas",
        tradeImpulso: resultado.ultimaVisitaTradeImpulso
          ? resultado.ultimaVisitaTradeImpulso.toLocaleDateString()
          : "Sin visitas",
      });
      ...
      console.error(
        `Error obteniendo últimas visitas por tipo para cliente ${rifCliente}:`,
        error
      );
      return {
        ultimaVisitaMerchandising: null,
   ...
   */
  const obtenerInformacionSeñalizacion = async (
    rifCliente: string
  ): Promise<InformacionSeñalizacion> => {
    try {
      console.log(
        `🔍 Buscando información de señalización para cliente RIF: ${rifCliente}`
      );
      ...
      if (visitas.length === 0) {
        console.log(
          `⚠️ No se encontraron visitas de merchandising para el cliente ${rifCliente}`
        );
        return {
          tieneSeñalizacion: false,
          estado: "Sin información",
        ...
        try {
          ...
          if (typeof data === "string") {
            try {
              ...
              console.log(
                "📝 Los datos no son JSON válido, tratando como texto"
              );
            }
          }
          ...
          if (data.señalizacion !== undefined) {
            ...
            console.log(
              `🎯 Campo directo 'señalizacion' encontrado:`,
              data.señalizacion,
              "→ parsed:",
              parsed
            );
            ...
            console.log(
              `🎯 Campo directo 'signage' encontrado:`,
              data.signage,
              "→ parsed:",
              parsed
            );
          ...
          if (estado === "Sin información" && data.hasSignage !== undefined) {
            ...
            console.log(
              `🎯 Campo directo 'hasSignage' encontrado:`,
              data.hasSignage,
              "→ parsed:",
              parsed
            );
          ...
          if (estado === "Sin información") {
            ...
            if (
              ...
              const seccionPreguntas =
                ...
                data.answers;
              console.log(
                `📋 Sección de preguntas encontrada:`,
                seccionPreguntas
              );
              ...
              for (const [pregunta, respuesta] of Object.entries(
                ...
                if (
                  preguntaLower.includes("señalizacion") ||
                  preguntaLower.includes("signage") ||
                  preguntaLower.includes("letrero") ||
                  preguntaLower.includes("cartel")
                ) {
                  console.log(
                    `🎯 Pregunta sobre señalización encontrada: ${pregunta} = ${respuesta}`
                  );
      ...
      console.error(
        `Error obteniendo información de señalización para cliente ${rifCliente}:`,
        error
      );
      return {
        tieneSeñalizacion: false,
  ...
  const loadClientes = useCallback(
    async (loadMore = false) => {
      try {
        ...
        console.log(
          `📄 Cargando clientes... ${loadMore ? "(más)" : "(inicial)"} - Límite: ${CLIENTS_PER_PAGE}`
        );
        ...
        console.log(
          `✅ Obtenidos ${querySnapshot.docs.length} clientes de Firestore`
        );
        ...
        console.log(
          `🔄 Procesando información básica para ${clientesData.length} clientes (sin señalización por ahora)...`
        );
        ...
        console.log(
          `✅ Carga básica completada: ${clientesBasicos.length} clientes listos`
        );
      } catch (error: any) {
    ...
    try {
      ...
      if (typeof window !== "undefined" && !navigator.onLine) {
        ...
        if (saveResult.success) {
          console.log(
            "✅ Cliente guardado offline exitosamente:",
            saveResult.visitaId
          );
          ...
          throw new Error(
            saveResult.error || "Error guardando cliente offline"
          );
        }
      } else {
  ...
  const resetForm = (cliente?: Cliente | null) => {
    ...
    setFormData({
      ...
      tipoVisitaPredeterminado:
        cliente?.tipoVisitaPredeterminado || "Merchandising",
    });
  ...
  const generateAutoMapping = (csvHeaders: string[]): ColumnMapping[] => {
    ...
    const mapSpecific = (
      original: string
    ): keyof CreateClienteData | "skip" | undefined => {
      switch (original) {
        case "CodigoCliente":
          return "rif";
        case "nombreCliente":
    ...
    return csvHeaders.map((header) => {
      ...
      const normalized = originalExact
        .toLowerCase()
        .replace(/\s+/g, "")
        .replace(/[^a-z0-9]/g, "");
      const specific = mapSpecific(originalExact);
      const matched =
        specific === undefined ? autoMappings[normalized] : specific;
  ...
  const processCSVData = (
    ...
    const mapTipo = (v?: string) => {
      if (!v) return "tienda";
      const m: Record<string, CreateClienteData["tipo"]> = {
        tienda: "tienda",
        store: "tienda",
        shop: "tienda",
    ...
    for (const row of csvData) {
      ...
      for (const mapping of mappings) {
        ...
        switch (mapping.clienteField) {
          case "region":
            clientePartial.region = clientePartial.sede
              ? assignRegionBySede(clientePartial.sede)
              : "Centro-capital";
            break;
          case "sede":
            // Preferir sede derivada desde estado
            const sedeFromEstado = mapSedeByEstado(
              clientePartial.estadoGeografico
            );
            if (sedeFromEstado) {
              clientePartial.sede = sedeFromEstado;
            } else {
              const normalized = raw.toLowerCase().trim();
              const fallbackMap: Record<string, Sede> = {
              ...
              clientePartial.sede =
                fallbackMap[normalized] || "GRUPO DISBATTERY";
            }
            break;
          case "tipo":
          ...
          case "ciudad":
            if (raw) clientePartial.ciudad = raw;
              ...
              clientePartial.ciudad = clientePartial.estadoGeografico
                ? ciudadFallback(clientePartial.estadoGeografico)
                : "";
            break;
          default:
            (clientePartial as any)[mapping.clienteField] = raw;
      ...
      if (clientePartial.nombre && clientePartial.direccion) {
        ...
        const regionFinal =
          clientePartial.region || assignRegionBySede(sedeFinal);
        processed.push({
  ...
  const updateColumnMapping = (
    index: number,
    field: keyof CreateClienteData | "skip"
  ) => {
    setBulkUpload((prev) => ({
      ...prev,
  ...
  const handlePreviewData = () => {
    ...
    if (processedData.length === 0) {
      toast({
        title: "Error",
        description:
          "No se encontraron datos válidos para procesar. Verifica el mapeo de columnas.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleConfigureVisitType = (cliente: Cliente) => {
    ...
    if (currentUser && !canAccessSede(currentUser, cliente.sede)) {
      alert(
        "No tienes permisos para configurar el tipo de visita de clientes de esta sede"
      );
      return;
    }
  ...
  const handleSaveVisitType = async () => {
    if (
      ...
      !selectedVisitType ||
      selectedVisitType === "sin_configurar"
    ) {
    ...
    try {
      const clienteRef = doc(
        getFirestoreClient(),
        "clientes",
        selectedClienteForVisitType.id
      );
      await updateDoc(clienteRef, {
  ...
  const getDiasSinVisita = (cliente: Cliente): number | null => {
    ...
    const fechaVisita =
      typeof cliente.lastVisitDate === "string"
  ...
  const loadClientesWithGPS = useCallback(async () => {
    ...
    try {
      ...
      const querySnapshot = await getDocs(q);
      console.log(
        `📊 Clientes obtenidos de Firestore: ${querySnapshot.docs.length}`
      );
      ...
      console.log(
        `📍 Clientes con GPS válido encontrados: ${clientesConGPS.length}`
      );
      console.log(
        `🏢 Distribución por sede:`,
        clientesConGPS.reduce(
          (acc, cliente) => {
  ...
  const filteredClientes = useMemo(() => {
    ...
    console.log(
      `👤 Usuario actual: ${currentUser?.fullName} - Sede: ${currentUser?.sede}`
    );
    ...
    const filtered = clientes.filter((cliente) => {
      ...
      if (currentUser && !canAccessSede(currentUser, cliente.sede)) {
        console.log(
          `🚫 Cliente ${cliente.nombre} EXCLUIDO por permisos - Sede cliente: ${cliente.sede}, Sede usuario: ${currentUser.sede}`
        );
        return false;
      }
      ...
      const matchesCity =
        filterCity === "todas" || cliente.ciudad === filterCity;
    ...
    console.log(
      `📍 Clientes con GPS válido en filtrados: ${filtered.filter((c) => c.position?.lat && c.position?.lng && c.position.lat !== 0 && c.position.lng !== 0).length}`
    );
  ...
  const prepareHeatmapData = () => {
    ...
    const clientesConGPS = filteredClientes.filter((cliente) => {
      ...
      if (!hasValidGPS) {
        console.log(
          `📍 Cliente sin GPS válido: ${cliente.nombre} - Coordenadas: (${cliente.position?.lat}, ${cliente.position?.lng})`
        );
      }
    ...
    console.log(
      `📍 Clientes con GPS válido: ${clientesConGPS.length} de ${filteredClientes.length}`
    );
    console.log(`🎯 Sede del usuario actual: ${currentUser?.sede}`);

    if (clientesConGPS.length === 0) {
      console.log(
        "⚠️ PROBLEMA: No hay clientes con coordenadas GPS válidas para mostrar en el mapa"
      );
      console.log("💡 POSIBLES CAUSAS:");
      console.log(
        "   1. Los clientes se cargaron sin coordenadas (optimización)"
      );
  ...
  const calculateMapCenter = () => {
    ...
    const validClientes = filteredClientes.filter(
      (cliente) => cliente.position?.lat && cliente.position?.lng
    );
    if (validClientes.length === 0) return { lat: 10.4806, lng: -66.9036 };

    const sumLat = validClientes.reduce(
      (sum, cliente) => sum + cliente.position.lat,
      0
    );
    const sumLng = validClientes.reduce(
      (sum, cliente) => sum + cliente.position.lng,
      0
    );
  ...
  return (
    ...
    >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-[85vw] md:max-w-2xl lg:max-w-4xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {isEditMode
                ...
                : "Agrega un nuevo punto de venta o cliente"}
            </DialogDescription>
          </DialogHeader>
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Formato: J seguido de 9 números
                </p>
              </div>
              ...
              <div>
                ...
                <Select
                  value={formData.tipo}
                  onValueChange={(
                    value: "tienda" | "distribuidor" | "cliente_especial"
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="Centro-capital">
                      Centro-capital (GRUPO DISBATTERY)
                    </SelectItem>
                    <SelectItem value="Centro-Los llanos">
                      Centro-Los llanos (BLITZ 2000)
                    </SelectItem>
                    <SelectItem value="Occidente">
                      Occidente (GRUPO VICTORIA)
                    </SelectItem>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: Sede) => {
                    ...
                    console.log(
                      "Ciudades disponibles para la sede:",
                      cities.length,
                      "ciudades"
                    );
              ...
              <div>
                ...
                <Combobox
                  ...
                  onValueChange={(value: string) => {
                    ...
                    console.log(
                      "Ciudades disponibles actualmente:",
                      availableCities.length
                    );
                    console.log(
                      "¿Ciudad está en la lista?",
                      availableCities.includes(value)
                    );
                ...
                {availableCities.length > 0 && (
                  <p className="text-xs text-gray-500 mt-1">
                    {availableCities.length} ciudades disponibles para{" "}
                    {formData.sede}
                  </p>
                )}
              </div>
            ...
            <div>
              <Label htmlFor="tipoVisitaPredeterminado">
                Tipo de Visita Predeterminado
              </Label>
              <Select
                ...
                onValueChange={(
                  ...
                  setFormData({
                    ...formData,
                    tipoVisitaPredeterminado:
                      value === "sin_configurar" ? undefined : value,
                  })
                }
              >
                <SelectTrigger>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Si se configura, los mercaderistas no tendrán que seleccionar el
                tipo de visita manualmente
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mb-3">
                📍 <strong>¡Nuevo!</strong> Si no tienes las coordenadas
                exactas, puedes dejar esto vacío. Los mercaderistas podrán
                capturar la ubicación GPS directamente cuando visiten al cliente
                en campo.
              </p>
              <MapSelector
                ...
                <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-center gap-2 text-blue-800">
                    ...
                    <span className="font-medium text-sm">
                      Tip: Sin coordenadas GPS
                    </span>
                  </div>
                  <p className="text-xs text-blue-700 mt-1">
                    Este cliente aparecerá marcado como "📍 Sin GPS" y los
                    mercaderistas podrán agregar la ubicación cuando lo visiten
                    por primera vez.
                  </p>
                </div>
              )}
      ...
      <Dialog
        open={isVisitTypeDialogOpen}
        onOpenChange={setIsVisitTypeDialogOpen}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Configurar Tipo de Visita</DialogTitle>
            <DialogDescription>
              Selecciona el tipo de visita predeterminado para{" "}
          ...
          <div className="space-y-4">
            ...
            <div className="text-sm text-gray-600 bg-blue-50 p-3 rounded-lg">
              ...
              <p>
                Al configurar un tipo de visita predeterminado, los
                mercaderistas no tendrán que seleccionarlo manualmente cuando
                visiten este cliente. El sistema automáticamente usará este tipo
                de visita.
              </p>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-4 border-t">
              ...
              <Button
                ...
                disabled={
                  !selectedVisitType || selectedVisitType === "sin_configurar"
                }
              >
            ...
            {bulkUpload.step === "upload" && (
              <div className="space-y-4">
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    <strong>Formato CSV esperado:</strong> CodigoCliente,
                    nombreCliente, rif, Direccion, Estado, Nombresucursal,
                    nombreVendedor.
                    <br />
                    <strong>🎯 Mapeo automático Estado → Sede → Región:</strong>
                    <br />• <strong>GRUPO DISBATTERY</strong> (Centro-capital):
                    Distrito Capital, Miranda, Vargas
                    <br />• <strong>DISBATTERY</strong> (Oriente): Aragua,
                    Anzoátegui, Bolívar, Monagas, Sucre, Nueva Esparta
                    <br />• <strong>BLITZ 2000</strong> (Centro-Los llanos):
                    Carabobo, Guárico, Lara, Yaracuy, Falcón, Zulia, Táchira,
                    Mérida, Trujillo
                    <br />• <strong>GRUPO VICTORIA</strong> (Oriente): Cojedes,
                    Portuguesa, Barinas, Apure, Amazonas, Delta Amacuro
                  </AlertDescription>
                </Alert>
              </div>
            )}
              ...
              <div className="space-y-4">
                  ...
                  {bulkUpload.columnMappings.map((mapping, index) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500">
                          Ejemplo:{" "}
                          {bulkUpload.csvData[0]?.[mapping.csvColumn] ||
                            "Sin datos"}
                        </p>
                      </div>
                      <div className="w-48">
                        <Select
                          ...
                          onValueChange={(value) =>
                            updateColumnMapping(
                              index,
                              value as keyof CreateClienteData | "skip"
                            )
                          }
                        >
                          <SelectTrigger>
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="estadoGeografico">
                              Estado
                            </SelectItem>
                            <SelectItem value="ciudad">Ciudad</SelectItem>
                            <SelectItem value="tipo">Tipo</SelectItem>
                            <SelectItem value="observaciones">
                              Observaciones
                            </SelectItem>
                          </SelectContent>
                        </Select>
                ...
                <div className="flex justify-between">
                  <Button
                    variant="outline"
                    onClick={() =>
                      setBulkUpload((prev) => ({ ...prev, step: "upload" }))
                    }
                  >
                    Volver
                  </Button>
                ...
                {bulkUpload.previewData.length > 0 ? (
                  <>
                          ...
                          {bulkUpload.previewData
                            ...
                            .map((cliente, index) => (
                              <TableRow key={index}>
                                ...
                                <TableCell>
                                  {cliente.estadoGeografico || "Sin estado"}
                                </TableCell>
                                <TableCell>
                    ...
                    <Alert>
                      ...
                      <AlertDescription>
                        Los datos se ven correctos. Los clientes se crearán con
                        GPS en (0,0) y los mercaderistas podrán actualizar las
                        coordenadas cuando visiten cada punto.
                      </AlertDescription>
                    </Alert>
                    ...
                    <div className="flex justify-between">
                      ...
                      >
                        Procesar Carga ({bulkUpload.previewData.length}{" "}
                        clientes)
                      </Button>
                    </div>
                  </>
                  ...
                  <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription>
                      No se encontraron datos válidos. Verifica que las columnas
                      estén mapeadas correctamente.
                    </AlertDescription>
                  </Alert>
                )}
              </div>

...
import {
  ...
  UserPermissions,
  canAccessSede,
  isAdminMaster as isUserAdminMaster,
  ADMIN_MASTER_EMAILS,
} from "@/services/auth";
...
const newUserSchema = z.object({
  ...
  role: z.enum(["Mercaderista", "Administrador", "Supervisor"], {
  ...
  region: z
    .enum(["Centro-capital", "Centro-Los llanos", "Occidente", "Oriente"])
    .optional(),
  sede: z
    .enum(["GRUPO DISBATTERY", "BLITZ 2000", "GRUPO VICTORIA", "DISBATTERY"])
    .optional(),
});
...
function UserManagementPageContent() {
  ...
  const [filterRole, setFilterRole] = useState<
    "todos" | "Mercaderista" | "Administrador" | "Supervisor"
  >("todos");
  ...
  const isCurrentUserAdminMaster = currentUser
    ? isUserAdminMaster(currentUser.email)
    : false;
    ...
    try {
      ...
      try {
        ...
        if (correosMercaderistas.length > 0) {
          const visitas =
            await obtenerUltimasVisitasUsuarios(correosMercaderistas);
  ...
  const onSubmit: SubmitHandler<NewUserFormData> = async (data) => {
    try {
      ...
      if (isAdminMasterUser) {
        ...
        console.log(
          "👑 AdminMaster detectado - creando usuario directamente SIN EMAIL DE APROBACIÓN"
        );
        ...
        console.log(
          "👤 Administrador regular - creando usuario con status pendiente"
        );
        ...
        if (typeof window !== "undefined" && !navigator.onLine) {
          console.log(
            "🔄 Modo Offline: Guardando usuario con offlineManager..."
          );
          ...
          if (saveResult.success) {
            console.log(
              "✅ Usuario guardado offline exitosamente:",
              saveResult.visitaId
            );
            ...
            throw new Error(
              saveResult.error || "Error guardando usuario offline"
            );
          }
        } else {
          ...
          console.log(
            "✅ Usuario guardado en Firestore con status pending_approval"
          );
  ...
  const handleSaveEditUser = async () => {
    ...
    try {
      ...
      if (typeof window !== "undefined" && !navigator.onLine) {
        console.log(
          "🔄 Modo Offline: Actualizando usuario con offlineManager..."
        );
        ...
        if (saveResult.success) {
          console.log(
            "✅ Usuario actualizado offline exitosamente:",
            saveResult.visitaId
          );
          ...
          throw new Error(
            saveResult.error || "Error actualizando usuario offline"
          );
        }
      } else {
        // Modo online: operación normal
        await setDoc(
          doc(getFirestoreClient(), "users", userToEdit.id),
          updateData,
          { merge: true }
        );
  ...
  const handleDeleteUser = async (user: User) => {
    ...
    if (
      ...
      !canAccessSede(currentUser, user.sede as Sede)
    ) {
      toast({
        title: "Acceso denegado",
  ...
  const filteredUsers = users.filter((user) => {
    ...
    if (
      currentUser &&
      !userPermissions?.isAdminMaster &&
      !canAccessSede(currentUser, user.sede)
    ) {
      return false;
    }
    ...
    const matchesRegion =
      filterRegion === "todos" || user.region === filterRegion;

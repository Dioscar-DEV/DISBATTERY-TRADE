...
export default function MyRoutePage() {
  ...
  const [previousStatus, setPreviousStatus] = useState<Route["status"] | null>(
    null
  );
  ...
  const [currentPointForLocation, setCurrentPointForLocation] =
    useState<RoutePoint | null>(null);
  const [isGettingLocation, setIsGettingLocation] = useState(false);

  // Estados para cliente cerrado
  const [isClienteSerradoDialogOpen, setIsClienteSerradoDialogOpen] =
    useState(false);
  const [currentPointForCerrado, setCurrentPointForCerrado] =
    useState<RoutePoint | null>(null);
  ...
  const [isClienteProspectoDialogOpen, setIsClienteProspectoDialogOpen] =
    useState(false);
      ...
      async (currentUser) => {
        if (currentUser) {
          ...
          try {
            ...
            await analytics.trackUserError(
  ...
  const checkAndAutoCompleteRoute = async (ruta: any, userId: string) => {
    try {
      ...
      const visitasDeHoy = visitasReales.filter((visita: any) => {
        const visitaDate =
          visita.createdAt instanceof Date
            ? visita.createdAt
            : (visita.createdAt as any)?.toDate
              ? (visita.createdAt as any).toDate()
      ...
      if (visitasDeHoy.length > 0) {
        ...
        const rifsVisitados = new Set(
          visitasDeHoy.map((v: any) => v.rifCliente).filter(Boolean)
        );
        const puntosConRif = ruta.points.filter(
          (p: any) => p.rif && p.rif.trim() !== ""
        );
        const puntosVisitados = puntosConRif.filter((p: any) =>
          rifsVisitados.has(p.rif)
        );
        ...
        const porcentajeVisitado =
          puntosConRif.length > 0
            ? (puntosVisitados.length / puntosConRif.length) * 100
            : 0;
        const deberiaAutoCompletar =
          porcentajeVisitado >= 80 || visitasDeHoy.length >= 2;

        console.log(
          `📊 Porcentaje visitado: ${porcentajeVisitado.toFixed(1)}%`
        );
  ...
  const loadTodaysEvents = async () => {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        const isDateInRange =
          fechaInicio <= todayString && fechaFin >= todayString;
        ...
        if (isDateInRange) {
          ...
          if (!isAssigned && nombreUsuario) {
            ...
            const encontrado = mercaderistas.find(
              (m: any) =>
                m &&
                typeof m === "string" &&
                m.toLowerCase().trim() === nombreNormalizado
            );
            if (encontrado) {
              isAssigned = true;
            }
  ...
  useEffect(() => {
    ...
    if (!user || !userData) {
      console.log("⏳ Esperando user y userData...", {
        hasUser: !!user,
    ...
    const loadRoutesWithStrategy = async () => {
      try {
        ...
        const debugInfo = await offlineManager.debugOfflineData(userData.uid);
        ...
        console.log(
          `📋 [${routeResult.source.toUpperCase()}] ${routeResult.routes.length} rutas encontradas`
        );
        ...
        if (routeResult.routes.length > 0) {
          ...
          localStorage.setItem(
            "todaysRoutesOffline",
            JSON.stringify(routeResult.routes)
          );
          ...
          try {
            ...
            if (email) {
              const visitasHoy = (
                await obtenerVisitas({ correoMercaderista: email })
              ).filter(
                (v: any) => format(v.createdAt, "yyyy-MM-dd") === todayString
              );
              if (
                visitasHoy.length > 0 &&
                finalRoutes[0] &&
                finalRoutes[0].status === "planificada"
              ) {
                console.log(
                  "🩹 Corrigiendo estado local a en_progreso por visitas existentes"
                );
            ...
            console.warn(
              "⚠️ No se pudo validar visitas para corregir estado local",
              e
            );
          }
        ...
        console.log(
          "📦 [FALLBACK] No hay rutas del dual loader, intentando localStorage..."
        );
        ...
        if (savedRoutes) {
          try {
            ...
            if (routes && routes.length > 0) {
              ...
              console.log(
                `📦 [LOCALSTORAGE] ${routes.length} rutas restauradas desde localStorage`
              );
              setIsLoading(false);
              return;
            }
            ...
            console.error(
              "❌ Error parseando rutas de localStorage:",
              parseError
            );
          }
        }
        ...
        if (navigator.onLine && userData.role === "Mercaderista") {
          console.log(
            "🌐 [FIREBASE DIRECT] Último intento: carga directa desde Firebase..."
          );
          ...
          try {
            ...
            if (directRoutes.length > 0) {
              console.log(
                `🎯 [FIREBASE DIRECT] ${directRoutes.length} rutas encontradas directamente`
              );
              localStorage.setItem(
                "todaysRoutesOffline",
                JSON.stringify(directRoutes)
              );
            ...
            console.error(
              "❌ Error en carga directa de Firebase:",
              directError
            );
          }
        }
        ...
        if (savedRoutes) {
          try {
            ...
            setTodaysRoutes(routes);
            console.log(
              "🆘 [EMERGENCY] Rutas restauradas de localStorage tras error crítico"
            );
          } catch {
            setTodaysRoutes([]);
          }
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      ...
      if (userData) {
        console.log(
          "🔄 [STORAGE EVENT] Eventos actualizados, recargando desde localStorage..."
        );
  ...
  useEffect(() => {
    if (user && userData) {
      ...
      if (navigator.onLine) {
        ...
        console.log(
          "🔄 Modo offline: saltando carga de métricas y datos históricos"
        );
  ...
  useEffect(() => {
    if (todaysRoutes.length > 0) {
      ...
      if (previousStatus && previousStatus !== currentStatus) {
        console.log(
          `🔄 Estado de ruta cambió: ${previousStatus} → ${currentStatus}`
        );
        ...
        switch (currentStatus) {
          case "en_progreso":
            title = "🚀 ¡Ruta iniciada!";
            description =
              "Tu ruta está ahora en proceso. ¡Comienza tus visitas!";
            break;
          case "completada":
  ...
  const loadMetrics = async () => {
    ...
    if (!user || !userData) {
      console.log("❌ No hay user o userData:", {
        user: !!user,
        userData: !!userData,
      });
      return;
    ...
    try {
      ...
      if (todasLasVisitas.length > 0) {
        ...
        const emailsUnicos = [
          ...new Set(todasLasVisitas.map((v: any) => v.direccionCorreo)),
        ];
        console.log("🔍 [DEBUG] Emails únicos en visitas:", emailsUnicos);

        // Mostrar todos los mercaderistas únicos para comparar
        const mercaderistasUnicos = [
          ...new Set(todasLasVisitas.map((v: any) => v.mercaderista)),
        ];
        console.log(
          "🔍 [DEBUG] Mercaderistas únicos en visitas:",
          mercaderistasUnicos
        );
      }
      ...
      if (allVisitas.length === 0 && userData.fullName) {
        console.log(
          "📊 No se encontraron visitas por email, intentando por nombre..."
        );
      ...
      console.log(
        "📊 Método de búsqueda usado:",
        allVisitas.length > 0
          ? allVisitas[0].direccionCorreo === user.email
            ? "Email"
            : "Nombre"
          : "Ninguno"
      );
      ...
      if (allVisitas.length === 0) {
        ...
        setMetrics({
          ...
          rutasCompletadas: todaysRoutes.filter(
      ...
      const visitasSemana = allVisitas.filter(
        (v: any) => v.createdAt >= startWeek
      ).length;
      const visitasMes = allVisitas.filter(
        (v: any) => v.createdAt >= startMonth
      ).length;
      ...
      const visitasUltimos30Dias = allVisitas.filter(
        (v: any) => v.createdAt >= last30Days
      ).length;
      const promedioVisitasDiarias =
        visitasUltimos30Dias > 0
          ? Math.round((visitasUltimos30Dias / 30) * 10) / 10
          : 0;

      // Contar rutas completadas hoy
      const rutasCompletadas = todaysRoutes.filter(
        (r) => r.status === "completada"
      ).length;
  ...
  const loadHistoricalData = async () => {
    ...
    try {
      ...
      const calendarInfo: RutaCalendario[] = fechasConsulta.map((fecha) => {
        const rutasDeLaFecha = allHistoricalRoutes.filter(
          (r) => r.date === fecha
        );
        const rutasCompletadas = rutasDeLaFecha.filter(
          (r) => r.status === "completada"
        ).length;
        ...
        let status: "completada" | "en_progreso" | "planificada" =
          "planificada";
      ...
      console.log("📅 Datos históricos cargados:", {
        ...
        díasConActividad: calendarInfo.filter(
          (c) => c.rutasCompletadas > 0 || c.visitasRealizadas > 0
        ).length,
      });
    } catch (error) {
  ...
  const routePoints: RoutePoint[] = todaysRoutes.flatMap((route) =>
    route.points.map((point) => ({
      ...point,
      _routeId: route.id,
      _routeName: route.mercaderista,
      _isEvent: false,
    }))
  );

  const eventPoints: RoutePoint[] = todaysEvents.map((evento) =>
    convertEventToPoint(evento)
  );
    ...
    return new Promise((resolve, reject) => {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          console.log(
            "📍 Precisión GPS:",
            accuracy ? `${accuracy.toFixed(0)} metros` : "Desconocida"
          );
          console.log(
            "📍 Fuente probable:",
            accuracy < 50 ? "GPS Satelital" : "Red Celular/WiFi"
          );
          console.log(
            "📍 Altitud:",
            altitude ? `${altitude.toFixed(0)}m` : "No disponible"
          );
          console.log(
            "📍 Timestamp:",
          ...
          if (accuracy && accuracy > 100) {
            console.warn(
              "⚠️ PRECISIÓN BAJA - Posible uso de red en lugar de GPS puro"
            );
          }
  ...
  const updateClientCoordinates = async (
    clienteRif: string,
    ...
    try {
      console.log(
        "📱 [OFFLINE] Guardando coordenadas en cola de sincronización..."
      );
      ...
      await offlineManager.queueVisitaForSync(updateData);

      console.log(
        "✅ ========= COORDENADAS EN COLA DE SINCRONIZACIÓN ========="
      );
  ...
  const handleUseCurrentLocation = async () => {
    ...
    try {
      ...
      if (currentPointForLocation.rif) {
        ...
        const updateSuccess = await updateClientCoordinates(
          currentPointForLocation.rif,
          currentPosition
        );
      ...
      if (error.message?.includes("User denied")) {
        errorMessage =
          "Acceso a ubicación denegado. Habilite la geolocalización para continuar.";
      } else if (error.message?.includes("Position unavailable")) {
        errorMessage =
          "Ubicación no disponible. Verifique que el GPS esté activado.";
  ...
  const continueWithVisit = async (point: RoutePoint) => {
    ...
    if (
      ...
      toast({
        title: "📱 Modo Offline",
        description:
          "Continuando en modo offline. Los datos se sincronizarán cuando haya conexión.",
      });
    }
    ...
    const finalPointId =
      point.id ||
      `punto-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    ...
    if (!parsed.pointId) {
      ...
      console.log(
        "✅ Datos guardados correctamente - PointId:",
        parsed.pointId
      );
    }
    ...
    router.push(
      `/visit-capture?pointId=${uniqueId}&pointName=${encodeURIComponent(point.name)}&pointType=${pointType}`
    );
  };
  ...
  const handleCompleteRoute = async () => {
    ...
    try {
      ...
      console.log(
        `📊 Visitas encontradas para completar ruta: ${visitasDeHoy.length}`
      );
      ...
      if (visitasDeHoy.length === 0) {
        toast({
          ...
          description:
            "Debes registrar al menos una visita antes de completar la ruta.",
          variant: "destructive",
        });
        return;
      }
  ...
  const guardarClienteCerrado = async () => {
    if (!currentPointForCerrado || !fotoCerrado || !razonCerrado.trim()) {
      toast({
        ...
        description:
          "Debe capturar una foto y especificar la razón del cierre.",
      });
      return;
    }
    ...
    try {
      ...
      const visitaId = await crearVisita({
        ...
        ubicacion: {
          lat: Number(
            (currentPointForCerrado.position as any)?.lat ??
              (currentPointForCerrado.position as any)?.latitude ??
              0
          ),
          lng: Number(
            (currentPointForCerrado.position as any)?.lng ??
              (currentPointForCerrado.position as any)?.longitude ??
              0
          ),
          direccion:
            (currentPointForCerrado as any).address ||
            (currentPointForCerrado as any).direccion ||
            "",
        },
      ...
      if (userData?.uid && currentPointForCerrado.id) {
        ...
        if (result.updated) {
          ...
          console.warn(
            "⚠️ No se pudo actualizar status en Firestore:",
            result.reason
          );
        }
      }
      ...
      toast({
        ...
        title: "Error",
        description:
          "No se pudo registrar el cliente cerrado. Intente nuevamente.",
      });
    }
  };
  ...
  const guardarClienteProspecto = async () => {
    if (
      !nombreProspecto.trim() ||
      !direccionProspecto.trim() ||
      !tipoNegocioProspecto
    ) {
      toast({
        variant: "destructive",
        title: "Datos Incompletos",
    ...
    try {
      ...
      const datosSheet = {
        ...
        "Latitud:": Number(
          gpsLocation?.latitude ?? (gpsLocation as any)?.lat ?? 0
        ),
        "Longitud:": Number(
          gpsLocation?.longitude ?? (gpsLocation as any)?.lng ?? 0
        ),
      ...
      toast({
        ...
        title: "Error",
        description:
          "No se pudo registrar el cliente prospecto. Intente nuevamente.",
      });
    }
  };
  ...
  const getCalendarDayColor = (rutaInfo: RutaCalendario) => {
    const tieneActividad =
      rutaInfo.rutasCompletadas > 0 || rutaInfo.visitasRealizadas > 0;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <CardTitle className="text-2xl font-bold">
                    ¡Hola,{" "}
                    {userData?.fullName || user?.displayName || "Mercaderista"}!
                  </CardTitle>
              ...
              <div className="text-right space-y-2">
                <div className="text-sm text-blue-100">
                  {userData?.sede && <p>📍 {userData.sede}</p>}
                  {userData?.region && <p>🗺️ {userData.region}</p>}
                </div>
                  ...
                  metrics.hasData ? (
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-semibold">
                              {metrics.promedioVisitasDiarias} visitas
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Rutas completadas:</span>
                            <span className="font-semibold">
                              {metrics.rutasCompletadas}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="text-center py-6">
                      ...
                      <p className="text-xs text-gray-500">
                        Aún no tienes visitas registradas. ¡Comienza tu primera
                        visita para ver tus métricas!
                      </p>
              ...
              <CardContent className="p-6">
                <Tabs defaultValue="ruta" className="w-full">
                  <TabsList className="grid w-full grid-cols-3 h-auto p-1">
                    <TabsTrigger
                      value="ruta"
                      className="text-xs sm:text-sm px-2 py-2 min-h-[2.5rem] flex items-center justify-center"
                    >
                                ...
                                <CardContent className="p-3 sm:p-4">
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <div className="flex-1 min-w-0">
                                      <h4 className="font-medium text-orange-900 text-sm sm:text-base truncate">
                                        {evento.nombreEvento}
                                      </h4>
                                      <p className="text-xs sm:text-sm text-orange-700 mt-1 truncate">
                                        {evento.direccion ||
                    ...
                    {allPoints.length > 0 ? (
                      <div className="space-y-4">
                        ...
                        <div className="bg-blue-50 border-l-4 border-l-blue-500 p-4 rounded-md">
                          ...
                          <p className="text-sm text-blue-700 mt-1">
                            Cada punto requiere un formulario independiente
                            según su tipo de visita
                          </p>
                        </div>
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-sm text-gray-600">
                                  Ruta de {todaysRoutes[0].mercaderista}
                                </p>
                              </div>
                              <Badge
                                className={`${getStatusColor(todaysRoutes[0].status)} text-sm px-3 py-1`}
                              >
                        ...
                        <div className="flex gap-4 mb-4 text-sm">
                          ...
                          {eventPoints.length > 0 && (
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-gray-600">
                                {eventPoints.length} evento
                                {eventPoints.length !== 1 ? "s" : ""}
                              </span>
                            </div>
                          )}
                        </div>
                            ...
                            return (
                              <li key={point.id}>
                                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                  <button
                                    onClick={() =>
                                      isClickable && handleStartVisit(point)
                                    }
                                      ...
                                      <CardContent className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 gap-3">
                                        <div className="flex items-start sm:items-center gap-2 sm:gap-4">
                                          ...
                                          <div className="min-w-0 flex-1">
                                            <p
                                              className={`font-bold text-base sm:text-lg flex flex-wrap items-center gap-1 sm:gap-2 ${
                                                isRouteCompleted
                                                  ? "text-gray-500"
                                                  : "text-gray-900"
                                              }`}
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="mt-2 text-gray-500">
                          Contacta a tu supervisor si crees que esto es un
                          error.
                        </p>
                      </div>
                    )}
                  ...
                  <TabsContent value="cliente" className="space-y-4 mt-6">
                    {selectedClient ? (
                      <div className="space-y-4">
                            ...
                            <CardContent className="space-y-3">
                              <div>
                                ...
                                <p className="font-mono">
                                  {selectedClient.rif || "No disponible"}
                                </p>
                              </div>
                              <div>
                                <label className="text-sm font-medium text-gray-500">
                              ...
                              <div>
                                ...
                                <p>
                                  {selectedClient.region || "No especificada"}
                                </p>
                              </div>
                              <div>
                                <label className="text-sm font-medium text-gray-500">
                                ...
                                <p>
                                  {selectedClient.sede || "No especificada"}
                                </p>
                              </div>
                              <div>
                                <label className="text-sm font-medium text-gray-500">
                                ...
                                <p>
                                  {selectedClient.ciudad || "No especificada"}
                                </p>
                              </div>
                            </CardContent>
                          </Card>
                        ...
                        <div className="flex justify-center pt-4">
                          ...
                          >
                            ...
                            {selectedClient.status === "pendiente"
                              ? "Iniciar Visita"
                              : "Ya Visitado"}
                          </Button>
                        </div>
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="mt-2 text-gray-500">
                          Haz clic en el botón de detalles junto a cualquier
                          cliente para ver su información completa.
                        </p>
                      </div>
                    )}
                  </TabsContent>
                        ...
                        <>
                            ...
                            {calendarData
                              ...
                              .map((rutaInfo, index) => {
                                ...
                                return (
                                  ...
                                  `}
                                    title={`${format(fecha, "EEEE, dd MMMM", { locale: es })}${
                                      tieneActividad
                          ...
                          <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div className="mt-3 p-2 bg-blue-50 rounded text-xs text-blue-700">
                              💡 <strong>Tip:</strong> Pasa el cursor sobre
                              cualquier día para ver detalles de tu actividad
                            </div>
                          </div>
                        </>
                      ) : (
                          ...
                          <div className="text-center space-y-3 sm:space-y-4">
                            ...
                            <div>
                              ...
                              <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6 px-2">
                                Registra clientes potenciales que encuentres
                                durante tu ruta para futuras visitas
                              </p>
                            </div>
                            <Button
      ...
      <Dialog
        open={isClienteSerradoDialogOpen}
        onOpenChange={setIsClienteSerradoDialogOpen}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Registra que el cliente {currentPointForCerrado?.name} está
              cerrado
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={razonCerrado} onValueChange={setRazonCerrado}>
                ...
                <SelectContent>
                  <SelectItem value="cerrado-horario">
                    Cerrado por horario
                  </SelectItem>
                  <SelectItem value="cerrado-vacaciones">
                    Cerrado por vacaciones
                  </SelectItem>
                  <SelectItem value="cerrado-inventario">
                    Cerrado por inventario
                  </SelectItem>
                  <SelectItem value="cerrado-mantenimiento">
                    Cerrado por mantenimiento
                  </SelectItem>
                  <SelectItem value="no-atiende">
                    No atiende/No responde
                  </SelectItem>
            ...
            <div className="space-y-2">
              <Label htmlFor="comentarios-cerrado">
                Comentarios Adicionales
              </Label>
              <Textarea
      ...
      <Dialog
        open={isClienteProspectoDialogOpen}
        onOpenChange={setIsClienteProspectoDialogOpen}
      >
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-sm">
                Foto del Establecimiento (Opcional)
              </Label>
              {fotoProspecto ? (
                <div className="space-y-2">
                  <img
            ...
            {/* Nombre del negocio */}
            <div className="space-y-2">
              <Label htmlFor="nombre-prospecto" className="text-sm font-medium">
                Nombre del Negocio *
              </Label>
              <Input
                id="nombre-prospecto"
            ...
            <div className="space-y-2">
              ...
              <Select
                value={tipoNegocioProspecto}
                onValueChange={setTipoNegocioProspecto}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="estacion-servicio">
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              El cliente <strong>{currentPointForLocation?.name}</strong> no
              tiene coordenadas GPS guardadas. Usa tu ubicación actual para
              registrar la visita:
            </DialogDescription>
          </DialogHeader>
            ...
            <Card className="cursor-pointer hover:bg-blue-50 border-blue-200 transition-colors">
              <CardContent className="p-4">
                ...
                <p className="text-xs text-gray-600 mt-2 text-center">
                  Utiliza el GPS de tu dispositivo para obtener tu ubicación
                  actual
                </p>
              </CardContent>
            </Card>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              <div className="flex items-start gap-2">
                ...
                <div className="text-xs text-amber-800">
                  ...
                  <p>
                    Las coordenadas GPS son necesarias para registrar la visita
                    correctamente y actualizar la base de datos del cliente para
                    futuras rutas.
                  </p>
                </div>
              </div>

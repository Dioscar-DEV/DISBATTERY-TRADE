...
export default function ObservacionesPage() {
  ...
  const [observacionShellFaltante, setObservacionShellFaltante] =
    useState<string>("");
  const [observacionQualidFaltante, setObservacionQualidFaltante] =
    useState<string>("");
  const [observacionesAdicionales, setObservacionesAdicionales] =
    useState<string>("");
  const [observacionesCompetencia, setObservacionesCompetencia] =
    useState<string>("");
  const [isSyncing, setIsSyncing] = useState(false);

  const handleSubmit = async () => {
    console.log(
      "🎯 ========== BOTÓN FINAL PRESIONADO - GUARDANDO FORMULARIO COMPLETO =========="
    );
    ...
    try {
      ...
      console.log(
        "🆔 [CRÍTICO ANÁLISIS] ClienteData RAW desde localStorage:",
        clienteDataRaw
      );
      ...
      if (clienteDataRaw) {
        try {
          ...
          console.log(
            "🆔 [CRÍTICO ANÁLISIS] ClienteData PARSED:",
            clienteDataFromStorage
          );
          console.log(
            "🆔 [CRÍTICO ANÁLISIS] PointId encontrado:",
            clienteDataFromStorage.pointId
          );
          console.log(
            "🆔 [CRÍTICO ANÁLISIS] Tipo de pointId:",
            typeof clienteDataFromStorage.pointId
          );
          console.log(
            "🆔 [CRÍTICO ANÁLISIS] PointId vacío?:",
            !clienteDataFromStorage.pointId
          );
          console.log(
            "🆔 [CRÍTICO ANÁLISIS] RIF del cliente:",
            clienteDataFromStorage.rif
          );
          console.log(
            "🆔 [CRÍTICO ANÁLISIS] Nombre del cliente:",
            clienteDataFromStorage.nombre
          );

          if (!clienteDataFromStorage.pointId) {
            console.error(
              "❌ [PROBLEMA CONFIRMADO] PointId está vacío al momento de enviar el formulario"
            );
            console.error(
              "❌ [CAUSA RAÍZ] Esto explica por qué se usa solo RIF como fallback"
            );
          } else {
            console.log(
              "✅ [ÉXITO] PointId correcto encontrado al momento del envío"
            );
          }
        } catch (error) {
        ...
        console.error(
          "❌ [PROBLEMA CRÍTICO] No hay clienteData en localStorage"
        );
      }
      ...
      const datosFormularioCompleto = localStorage.getItem(
      ...
      console.log(
        "🔍 RIF en clienteDataFromStorage:",
        clienteDataFromStorage.rif
      );
      console.log(
        "🔍 Nombre en clienteDataFromStorage:",
        clienteDataFromStorage.nombre
      );
      ...
      if (clienteDataFromStorage) {
        ...
        console.log(
          "🗺️ GPS desde clienteData:",
          clienteDataFromStorage.position
        );
        ...
        if (!cliente) {
          ...
          console.log(
            "✅ Cliente obtenido completamente desde clienteData:",
            cliente
          );
        } else {
          ...
          cliente = {
            ...
            hasSignage:
              clienteDataFromStorage.hasSignage !== undefined
                ? clienteDataFromStorage.hasSignage
                : cliente.hasSignage,
            signagePhoto:
              clienteDataFromStorage.signagePhoto !== undefined
                ? clienteDataFromStorage.signagePhoto
      ...
      let rifCliente =
        cliente?.rif || cliente?.rifCliente || cliente?.clientRif || "";
      ...
      console.log(
        "🗺️ UBICACIÓN/COORDENADAS DEL CLIENTE:",
        cliente?.position || cliente?.ubicacion
      );
      console.log(
        "🗺️ LATITUD:",
        cliente?.position?.lat || cliente?.position?.latitude
      );
      console.log(
        "🗺️ LONGITUD:",
        cliente?.position?.lng || cliente?.position?.longitude
      );
      ...
      console.log(
        "🗺️ UBICACIÓN PREPARADA PARA CREARVISITA:",
        ubicacionParaVisita
      );
      ...
      if (!rifCliente || !nombreEstablecimiento) {
        ...
        toast({
          ...
          description:
            "No se encontró el RIF o el nombre del establecimiento del cliente. Reinicie el proceso o seleccione correctamente el cliente.",
        });
        setIsSyncing(false);
        return;
      }
      ...
      console.log(
        "✅ Datos del cliente válidos, procediendo a guardar visita..."
      );
      ...
      if (!datosAcumulados.shellMerchandising) {
        toast({
          ...
          description:
            "No se encontraron datos de Shell Merchandising. Complete el formulario.",
        });
        return;
      }
      ...
      console.log(
        "🚩 clienteDataFromStorage.signagePhoto:",
        clienteDataFromStorage?.signagePhoto ? "SÍ CAPTURADA" : "NO CAPTURADA"
      );
      console.log("🚩 cliente.hasSignage (final):", cliente.hasSignage);
      console.log(
        "🚩 cliente.signagePhoto (final):",
        cliente.signagePhoto ? "SÍ CAPTURADA" : "NO CAPTURADA"
      );
      console.log(
        "🔧 CORRECCIÓN: Datos de signage desde clienteData:",
        signageData
      );
      ...
      console.log("🔧 DATOS DE SEÑALIZACIÓN AGREGADOS A datosAcumulados:", {
        ...
        signagePhoto: datosAcumulados.signagePhoto
          ? "SÍ CAPTURADA"
          : "NO CAPTURADA",
      });
      ...
      console.log(
        "🗺️ CLIENTE CON GPS ACTUALIZADO EN datosAcumulados.clienteData:",
        cliente.position
      );
      ...
      console.log(
        "  - signageData.signagePhoto existe:",
        !!signageData.signagePhoto
      );
      console.log(
        "  - Longitud de la imagen:",
        signageData.signagePhoto ? signageData.signagePhoto.length : 0
      );
      console.log(
        "  - Es base64 válido:",
        signageData.signagePhoto
          ? signageData.signagePhoto.startsWith("data:image/")
          : false
      );
      ...
      if (
        shellData.afichesColocadosShell &&
        Array.isArray(shellData.afichesColocadosShell)
      ) {
        shellData.afichesColocadosShell.forEach(
          (afiche: any, index: number) => {
            if (afiche.foto && afiche.foto.startsWith("data:image/")) {
              imagesToUpload.push({
                base64: afiche.foto,
                path: `merchandising/shell/${cliente.rif}/afiches`,
                prefix: `afiche_shell_${index}_${
                  afiche.tipo
                    ?.replace(/[^a-zA-Z0-9]/g, "_")
                    .toLowerCase()
                    .substring(0, 20) || "desconocido"
                }`,
              });
            }
          }
      ...
      if (datosAcumulados.shellMaterialInterno) {
        ...
        console.log(
          ...
          !!materialData.fotoExhibidoresShell
        );
        console.log(
          "  - fotoBanderinesShell:",
          !!materialData.fotoBanderinesShell
        );
        console.log(
          "  - fotoAvisoAcrilicoShell:",
          !!materialData.fotoAvisoAcrilicoShell
        );
        console.log(
          "  - afichesColocadosShell:",
          materialData.afichesColocadosShell?.length || 0
        );

        if (
          materialData.fotoExhibidoresShell &&
          materialData.fotoExhibidoresShell.startsWith("data:image/")
        ) {
        ...
        if (
          materialData.fotoBanderinesShell &&
          materialData.fotoBanderinesShell.startsWith("data:image/")
        ) {
        ...
        if (
          materialData.fotoAvisoAcrilicoShell &&
          materialData.fotoAvisoAcrilicoShell.startsWith("data:image/")
        ) {
        ...
        if (
          materialData.afichesColocadosShell &&
          Array.isArray(materialData.afichesColocadosShell)
        ) {
          console.log(
            "✅ PROCESANDO afiches individuales de Shell Material Interno"
          );
          materialData.afichesColocadosShell.forEach(
            (afiche: any, index: number) => {
              if (afiche.foto && afiche.foto.startsWith("data:image/")) {
                console.log(
                  `✅ AGREGANDO afiche Shell Material ${index + 1}: ${afiche.tipo}`
                );
                imagesToUpload.push({
                  base64: afiche.foto,
                  path: `merchandising/shell/${cliente.rif}/afiches_material`,
                  prefix: `afiche_material_shell_${index}_${
                    afiche.tipo
                      ?.replace(/[^a-zA-Z0-9]/g, "_")
                      .toLowerCase()
                      .substring(0, 20) || "desconocido"
      ...
      if (datosAcumulados.qualidMerchandising) {
        ...
        if (
          qualidData.afichesColocadosQualid &&
          Array.isArray(qualidData.afichesColocadosQualid)
        ) {
          qualidData.afichesColocadosQualid.forEach(
            (afiche: any, index: number) => {
              if (afiche.foto && afiche.foto.startsWith("data:image/")) {
                imagesToUpload.push({
                  base64: afiche.foto,
                  path: `merchandising/qualid/${cliente.rif}/afiches`,
                  prefix: `afiche_qualid_${index}_${
                    afiche.tipo
                      ?.replace(/[^a-zA-Z0-9]/g, "_")
                      .toLowerCase()
                      .substring(0, 20) || "desconocido"
      ...
      console.log(
        "  - fotoPromotorasShell:",
        !!datosAcumulados.fotoPromotorasShell
      );
      console.log(
        "  - fotoImpulsoQualid:",
        !!datosAcumulados.fotoImpulsoQualid
      );
      console.log(
        "  - fotoPromotorasQualid:",
        !!datosAcumulados.fotoPromotorasQualid
      );

      if (
        datosAcumulados.fotoImpulsoShell &&
        datosAcumulados.fotoImpulsoShell.startsWith("data:image/")
      ) {
      ...
      if (
        datosAcumulados.fotoPromotorasShell &&
        datosAcumulados.fotoPromotorasShell.startsWith("data:image/")
      ) {
      ...
      if (
        datosAcumulados.fotoImpulsoQualid &&
        datosAcumulados.fotoImpulsoQualid.startsWith("data:image/")
      ) {
      ...
      if (
        datosAcumulados.fotoPromotorasQualid &&
        datosAcumulados.fotoPromotorasQualid.startsWith("data:image/")
      ) {
      ...
      console.log(
        `📊 Total de imágenes preparadas para subir: ${imagesToUpload.length}`
      );
      imagesToUpload.forEach((img, index) => {
        console.log(
          `📷 Imagen ${index + 1}: ${img.prefix} (${img.base64 ? "Válida" : "Inválida"})`
        );
      });
      ...
      if (imagesToUpload.length > 0) {
        console.log(
          `📤 Subiendo ${imagesToUpload.length} imágenes a Firebase Storage...`
        );
      ...
      if (
        ...
        console.log(
          `🚩 FOTO SEÑALIZACIÓN - URL mapeada [${urlIndex}]:`,
          fotoSeñalizacionUrl
        );
        console.log(
          `🚩 FOTO SEÑALIZACIÓN - hasSignage:`,
          signageData.hasSignage
        );
        console.log(
          `🚩 FOTO SEÑALIZACIÓN - ¿Debería tener foto?:`,
          signageData.hasSignage === "Yes"
        );
        urlIndex++;
      } else {
        console.log(
          "🚩 FOTO SEÑALIZACIÓN - SALTANDO mapeo (no existe foto válida)"
        );
        console.log(
          `🚩 FOTO SEÑALIZACIÓN - signageData.signagePhoto:`,
          signageData.signagePhoto ? "EXISTE PERO NO ES BASE64" : "NO EXISTE"
        );
        console.log(
          `🚩 FOTO SEÑALIZACIÓN - hasSignage:`,
          signageData.hasSignage
      ...
      if (
        ...
        const afichesConFoto = shellData.afichesColocadosShell.filter(
          (afiche: any) => afiche.foto && afiche.foto.startsWith("data:image/")
        );
      ...
      if (datosAcumulados.shellMaterialInterno) {
        ...
        if (
          ...
          console.log(
            `  - URL de exhibidores Shell mapeada [${urlIndex}]:`,
            fotoExhibidoresShellUrl
          );
          urlIndex++;
        }
        ...
        if (
          ...
          console.log(
            `  - URL de banderines Shell mapeada [${urlIndex}]:`,
            fotoBanderinesShellUrl
          );
          urlIndex++;
        }
        ...
        if (
          ...
          console.log(
            `  - URL de aviso acrílico Shell mapeada [${urlIndex}]:`,
            fotoAvisoAcrilicoShellUrl
          );
          urlIndex++;
        }
        ...
        if (
          ...
          const afichesConFoto = materialData.afichesColocadosShell.filter(
            (afiche: any) =>
          ...
          for (let i = 0; i < afichesConFoto.length; i++) {
            if (fotosUrls[urlIndex]) {
              ...
              console.log(
                `  - URL de afiche Shell Material ${i + 1} mapeada [${urlIndex}]:`,
                fotosUrls[urlIndex]
              );
            }
            urlIndex++;
      ...
      if (datosAcumulados.qualidMerchandising) {
        ...
        if (
          ...
          const afichesConFoto = qualidData.afichesColocadosQualid.filter(
            (afiche: any) =>
              afiche.foto && afiche.foto.startsWith("data:image/")
          );
      ...
      if (
        ...
        console.log(
          `  - URL de impulso Shell mapeada [${urlIndex}]:`,
          fotoImpulsoShellUrl
        );
        urlIndex++;
      }
      ...
      if (
        ...
        console.log(
          `  - URL de promotoras Shell mapeada [${urlIndex}]:`,
          fotoPromotorasShellUrl
        );
        urlIndex++;
      }
      ...
      if (
        ...
        console.log(
          `  - URL de impulso Qualid mapeada [${urlIndex}]:`,
          fotoImpulsoQualidUrl
        );
        urlIndex++;
      }
      ...
      if (
        ...
        console.log(
          `  - URL de promotoras Qualid mapeada [${urlIndex}]:`,
          fotoPromotorasQualidUrl
        );
        urlIndex++;
      }

      console.log(
        `✅ URLs mapeadas correctamente. Total URLs utilizadas: ${urlIndex} de ${fotosUrls.length}`
      );
      console.log(
        `📷 Fotos de afiches Shell Merchandising: ${fotosAfichesShellUrls.length} URLs`
      );
      console.log(
        `📷 Fotos de afiches Shell Material Interno: ${fotosAfichesShellMaterialUrls.length} URLs`
      );
      console.log(
        `📷 Fotos de afiches Qualid: ${fotosAfichesQualidUrls.length} URLs`
      );
      console.log(
        `📷 Fotos de Shell Material: exhibidores=${!!fotoExhibidoresShellUrl}, banderines=${!!fotoBanderinesShellUrl}, acrilico=${!!fotoAvisoAcrilicoShellUrl}`
      );
      console.log(
        `📷 Fotos de Trade: impulsoShell=${!!fotoImpulsoShellUrl}, promotorasShell=${!!fotoPromotorasShellUrl}, impulsoQualid=${!!fotoImpulsoQualidUrl}, promotorasQualid=${!!fotoPromotorasQualidUrl}`
      );
      ...
      const respuestas: RespuestasMerchandising = {
        ...
        fotoSeñalizacion:
          fotoSeñalizacionUrl ||
          (signageData.hasSignage === "Yes" ? "No capturada" : ""),
        ...
        clienteTieneExhibidoresShell:
          datosAcumulados.shellMaterialInterno?.tieneExhibidoresShell || false,
        ...
        afichesFerrari2023:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("FERRARI 2023")
          )?.cantidad || 0,
        afichesHX8:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("HX8")
          )?.cantidad || 0,
        afichesProductosPremium2024:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("PRODUCTOS PREMIUM 2024")
          )?.cantidad || 0,
        afichesShellFamilia2023:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("SHELL FAMILIA 2023")
          )?.cantidad || 0,
        afichesShellHX7:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("SHELL HX7")
          )?.cantidad || 0,
        afichesTablaAplicacionShell:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("TABLA DE APLICACION SHELL")
          )?.cantidad || 0,
        aficheShellGadus2021:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("SHELL GADUS 2021")
          )?.cantidad || 0,
        aficheShellHelix:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("SHELL HELIX")
          )?.cantidad || 0,
        aficheShellRimula:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("SHELL RIMULA")
          )?.cantidad || 0,
        aficheShellAdvance:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("SHELL ADVANCE")
          )?.cantidad || 0,
        aficheShell5W30:
          shellData.afichesColocadosShell?.find((a: any) =>
            a.tipo?.includes("SHELL 5W-30")
          )?.cantidad || 0,
        ...
        fotosAfichesShellMaterial:
          fotosAfichesShellMaterialUrls.join(" | ") || "",

        // ✅ CORRECCIÓN: Banderines Shell - datos de shell-material-interno con URL de Firebase
        colocasteBanderinesShell:
          datosAcumulados.shellMaterialInterno?.colocoBanderinesShell || false,
        totalBanderinesShell:
          datosAcumulados.shellMaterialInterno?.cantidadTirasBanderinesShell ||
          0,
        ...
        clienteTieneAvisoAcrilicoShell:
          datosAcumulados.shellMaterialInterno?.colocoAvisoAcrilicoShell ||
          false,
        ...
        hicistePlanogramaQualid:
          datosAcumulados.qualidMerchandising?.hicistePlanogramaQualid || false,
        fotoAntesQualid: fotoAntesPlanogramaQualidUrl,
        fotoDespuesQualid: fotoDespuesPlanogramaQualidUrl,
        totalCenefasQualid:
          datosAcumulados.qualidMerchandising?.totalCenefasQualid || 0,
        totalBolsasQualid:
          datosAcumulados.qualidMerchandising?.bolsasQualidCarro || 0,

        // Afiches Qualid - datos con cantidades individuales
        afiches_FiltrosFluidos2024:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("FILTROS Y FLUIDOS 2024")
          )?.cantidad || 0,
        afichesQualidCaucho2023:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("QUALID CAUCHO 2023")
          )?.cantidad || 0,
        afichesQualidCaucho2024:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("QUALID CAUCHO 2024")
          )?.cantidad || 0,
        afichesQualidCuidadoAutomotriz2022:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("CUIDADO AUTOMOTRIZ 2022")
          )?.cantidad || 0,
        afichesQualidFF2022:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("QUALID FF 2022")
          )?.cantidad || 0,
        afichesQualidFiltros2022:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("QUALID FILTROS 2022")
          )?.cantidad || 0,
        afichesQualidMantenimiento2022:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("QUALID MANTENIMIENTO 2022")
          )?.cantidad || 0,
        afichesQualidTablaCrossReference2024:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("CROSS REFERENCE")
          )?.cantidad || 0,
        afichesQualidTablaAplicacion:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("TABLA DE APLICACIÓN")
          )?.cantidad || 0,
        afichesQualidTablaFiltroAutomotriz2024:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("FILTRO AUTOMOTRIZ 2024")
          )?.cantidad || 0,
        aficheQualidFiltrosAutomotriz:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("FILTROS AUTOMOTRIZ")
          )?.cantidad || 0,
        aficheQualidFamilyCarCare:
          datosAcumulados.qualidMerchandising?.afichesColocadosQualid?.find(
            (a: any) => a.tipo?.includes("FAMILY CAR CARE")
          )?.cantidad || 0,
        ...
        colocasteExhibidoresCauchosQualid:
          datosAcumulados.qualidMerchandising?.exhibidoresCauchoQualid?.length >
            0 || false,
        totalExhibidorCauchoPequeño:
          datosAcumulados.qualidMerchandising?.exhibidoresCauchoQualid?.find(
            (e: any) => e.tipo?.includes("Pequeño")
          )?.cantidad || 0,
        totalExhibidorCauchoGrande:
          datosAcumulados.qualidMerchandising?.exhibidoresCauchoQualid?.find(
            (e: any) => e.tipo?.includes("Grande")
          )?.cantidad || 0,
        ...
        observacionesShell:
          reportesFinalesData?.reporteShellFaltante || observacionShellFaltante,
        observacionesQualid:
          reportesFinalesData?.reporteQualidFaltante ||
        ...
        observaciones: [
          ...
          signageData.hasSignage === "Yes"
            ? "Sí"
            : signageData.hasSignage === "No"
              ? "No"
              : "No respondido",
          shellData.hicistePlanogramaShell === true
            ? "Sí"
            : shellData.hicistePlanogramaShell === false
              ? "No"
              : "No respondido",
          shellData.clienteTieneStickerShell === true
            ? "Sí"
            : shellData.clienteTieneStickerShell === false
              ? "No"
              : "No respondido",
          shellData.colocasteStickerShell === true
            ? "Sí"
            : shellData.colocasteStickerShell === false
              ? "No"
              : "No respondido",
          ...
          ventasData?.shell?.huboVentas === true
            ? "Sí"
            : ventasData?.shell?.huboVentas === false
              ? "No"
              : "No respondido",
          ventasData?.qualid?.huboVentas === true
            ? "Sí"
            : ventasData?.qualid?.huboVentas === false
              ? "No"
              : "No respondido",
          `${fotosUrls.length}`,
      ...
      if (observacionesAdicionales.trim()) {
        observacionesCombinadasArray.push(
          `Comentarios adicionales: ${observacionesAdicionales.trim()}`
        );
      }
      if (observacionesCompetencia.trim()) {
        observacionesCombinadasArray.push(
          `Observaciones competencia: ${observacionesCompetencia.trim()}`
        );
      }
      ...
      if (reportesFinalesData?.reporteComentariosAdicionales?.trim()) {
        observacionesCombinadasArray.push(
          `Reporte final: ${reportesFinalesData.reporteComentariosAdicionales.trim()}`
        );
      }
      ...
      if (ventasData) {
        ...
        if (ventasData.shell?.huboVentas !== undefined) {
          resumenVentas.push(
            `Ventas SHELL: ${ventasData.shell.huboVentas ? "Sí" : "No"}`
          );
        }
        if (ventasData.qualid?.huboVentas !== undefined) {
          resumenVentas.push(
            `Ventas QUALID: ${ventasData.qualid.huboVentas ? "Sí" : "No"}`
          );
        }
        if (resumenVentas.length > 0) {
          observacionesCombinadasArray.push(
            `Resumen ventas: ${resumenVentas.join(" | ")}`
          );
        }
      }
        ...
        reportesFinales: reportesFinalesData
          ? {
              reporteShellFaltante:
                reportesFinalesData.reporteShellFaltante || null,
              reporteQualidFaltante:
                reportesFinalesData.reporteQualidFaltante || null,
              comentariosAdicionales:
                reportesFinalesData.reporteComentariosAdicionales || null,
            }
          : null,
      ...
      if (!navigator.onLine) {
        ...
        const payload = {
          ...
          respuestas,
          observacionesAdicionales: Array.isArray(respuestas.observaciones)
            ? respuestas.observaciones.join(" | ")
            : respuestas.observaciones || "Sin observaciones",
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        toast({
          ...
          description:
            "La visita se guardó localmente y se sincronizará al volver la conexión.",
        });
      ...
      console.log(
        "🎉 ========== GUARDADO EXITOSO - ESPERANDO 3 SEGUNDOS PARA LOGS =========="
      );
      console.log("🎉 [ÉXITO] Visita guardada con ID:", visitaId);
      console.log(
        "🎉 [ÉXITO] Revisa los logs anteriores para confirmar que pointId se procesó correctamente"
      );
      console.log(
        "🎉 [REDIRECCIÓN] Navegando a página de éxito en 3 segundos..."
      );

      setTimeout(() => {
        console.log(
          "🔄 [REDIRECCIÓN] Ejecutando router.push a /registro-exitoso"
        );
      ...
      const observacionesData = JSON.parse(
        localStorage.getItem("observacionesData") || "[]"
      );
      observacionesData.push(dataFallback);
      localStorage.setItem(
        "observacionesData",
        JSON.stringify(observacionesData)
      );
      ...
      toast({
        ...
        description:
          "Se guardaron las observaciones localmente. Los datos se enviarán cuando haya conexión.",
      });

      // ✅ DELAY ANTES DE REDIRECCIÓN PARA CAPTURAR LOGS DE ERROR
      console.log(
        "❌ ========== ERROR EN GUARDADO - ESPERANDO 3 SEGUNDOS PARA LOGS =========="
      );
      console.log("❌ [ERROR] Error completo capturado:", error);
      console.log(
        "❌ [ERROR] Revisa los logs anteriores para identificar el problema con pointId"
      );
      console.log(
        "❌ [REDIRECCIÓN] Navegando a página de éxito en 3 segundos a pesar del error..."
      );

      setTimeout(() => {
        console.log(
          "🔄 [REDIRECCIÓN ERROR] Ejecutando router.push a /registro-exitoso"
        );
        router.push("/registro-exitoso");
      }, 3000); // 3 segundos de delay
    } finally {
      ...
      >
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Complete sus observaciones para finalizar la visita.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
                ...
                <div className="mt-2 space-y-1">
                  {datosParaMostrar.ventas && (
                    <>
                      <div className="text-sm">
                        <span className="font-medium">Ventas SHELL:</span>{" "}
                        {datosParaMostrar.ventas.shell?.huboVentas
                          ? "Sí"
                          : "No"}
                      </div>
                      <div className="text-sm">
                        <span className="font-medium">Ventas QUALID:</span>{" "}
                        {datosParaMostrar.ventas.qualid?.huboVentas
                          ? "Sí"
                          : "No"}
                      </div>
                    </>
                  ...
                  {datosParaMostrar.reportesFinales && (
                    <div className="text-xs text-gray-600 mt-2 pt-2 border-t">
                      <div>
                        ✓ Reporte SHELL:{" "}
                        {datosParaMostrar.reportesFinales.reporteShellFaltante
                          ? "Completado"
                          : "Vacío"}
                      </div>
                      <div>
                        ✓ Reporte QUALID:{" "}
                        {datosParaMostrar.reportesFinales.reporteQualidFaltante
                          ? "Completado"
                          : "Vacío"}
                      </div>
                      <div>
            ...
            <div>
              <Label htmlFor="obs-shell-faltante" className="text-sm">
                Coloca aquí tus observaciones de producto faltante y cualquier
                comentario adicional para la cartera de productos SHELL:
              </Label>
              <Textarea
            ...
            <div>
              <Label htmlFor="obs-qualid-faltante" className="text-sm">
                Coloca aquí tus observaciones de producto faltante y cualquier
                comentario adicional para la cartera de productos QUALID:
              </Label>
              <Textarea
          ...
          <CardFooter>
            ...
            >
              {isSyncing
                ? "Guardando Visita Completa..."
                : "Finalizar y Guardar Visita Completa"}
            </Button>
          </CardFooter>
        </Card>
      </div>

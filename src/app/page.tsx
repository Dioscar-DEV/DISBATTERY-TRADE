...
export default function Home() {
    ...
    try {
      ...
      if (result.success) {
        if (result.redirect.shouldPreload) {
          ...
          console.log('🌐 [LOGIN] Redirigiendo inmediatamente para usuario administrativo');
          router.push(result.redirect.path);
        }
      } else {
        setPreloadError(result.error || 'Error ejecutando estrategia post-login');
        setPendingRedirect(result.redirect.path);
      }
    } catch (error) {
      console.error('❌ Error en estrategia post-login:', error);
      setPreloadError(error instanceof Error ? error.message : 'Error desconocido');
    }
  };

  const handleForgotPassword = async () => {
    if (!username) {
      showToast('Por favor, ingrese su dirección de correo electrónico en el campo Usuario.');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (userData) {
          ...
          console.warn('No se encontraron datos del usuario en Firestore, usando defaults');
          showToast('¡Login exitoso! Preparando datos offline...');
          if (typeof window !== 'undefined') {
            localStorage.setItem('userLoggedIn', 'true'); // ✅ Marcar como logueado
            localStorage.setItem('merchandiserLoggedIn', 'true');
  ...
  return (
    <div>
      ...
      {/* Componente de progreso de precarga de datos */}
      <DataPreloadProgress

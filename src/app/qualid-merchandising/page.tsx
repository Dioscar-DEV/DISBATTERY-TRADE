...
export default function QualidMerchandising() {
  const [hicistePlanogramaQualid, setHicistePlanogramaQualid] =
    useState<string>("");
  const [fotoAntesPlanogramaQualid, setFotoAntesPlanogramaQualid] = useState<
    string | null
  >(null);
  const [fotoDespuesPlanogramaQualid, setFotoDespuesPlanogramaQualid] =
    useState<string | null>(null);

  const [totalCenefasQualid, setTotalCenefasQualid] = useState<number | null>(
    null
  );
  const [bolsasQualidCarro, setBolsasQualidCarro] = useState<number | null>(
    null
  );

  const [stickerQualid, setStickerQualid] = useState<number | null>(null);

  const [afichesQualidAgregados, setAfichesQualidAgregados] = useState<
    AficheColocado[]
  >([]);
  const [currentTipoAficheQualid, setCurrentTipoAficheQualid] =
    useState<string>("");
  const [currentCantidadAficheQualid, setCurrentCantidadAficheQualid] =
    useState<string>("");
  const [currentFotoAficheQualid, setCurrentFotoAficheQualid] = useState<
    string | null
  >(null);

  const [
    exhibidoresCauchoQualidAgregados,
    setExhibidoresCauchoQualidAgregados,
  ] = useState<ExhibidorCauchoColocado[]>([]);
  const [
    currentTipoExhibidorCauchoQualid,
    setCurrentTipoExhibidorCauchoQualid,
  ] = useState<string>("");
  const [
    currentCantidadExhibidorCauchoQualid,
    setCurrentCantidadExhibidorCauchoQualid,
  ] = useState<string>("");
  const [fotoExhibidoresCauchoQualid, setFotoExhibidoresCauchoQualid] =
    useState<string | null>(null);
  ...
  const handleTotalCenefasQualidChange = (
  ...
  const handleBolsasQualidCarroChange = (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    const value = e.target.value;
  ...
  const handleStickerQualidChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setStickerQualid(value === "" ? null : parseInt(value));
  };
  ...
  const handleAddAficheQualid = () => {
    ...
    if (
      currentCantidadAficheQualid === "" ||
      currentCantidadAficheQualid === null
    ) {
      toast({
        variant: "destructive",
    ...
    setAfichesQualidAgregados([
      ...afichesQualidAgregados,
      {
  ...
  const handleRemoveAficheQualid = (indexToRemove: number) => {
    setAfichesQualidAgregados(
      afichesQualidAgregados.filter((_, index) => index !== indexToRemove)
    );
  };
  ...
  const handleAddExhibidorCauchoQualid = () => {
    if (!currentTipoExhibidorCauchoQualid) {
      toast({
        ...
        description:
          "Por favor, seleccione un tipo de exhibidor de caucho Qualid.",
      });
      return;
    }
    if (
      currentCantidadExhibidorCauchoQualid === "" ||
      currentCantidadExhibidorCauchoQualid === null
    ) {
      toast({
        variant: "destructive",
        title: "Cantidad Requerida",
    ...
    setExhibidoresCauchoQualidAgregados([
      ...exhibidoresCauchoQualidAgregados,
      { tipo: currentTipoExhibidorCauchoQualid, cantidad: cantidadNum },
    ]);
    setCurrentTipoExhibidorCauchoQualid("");
    setCurrentCantidadExhibidorCauchoQualid("");
  };
  ...
  const handleRemoveExhibidorCauchoQualid = (indexToRemove: number) => {
    setExhibidoresCauchoQualidAgregados(
      exhibidoresCauchoQualidAgregados.filter(
  ...
  const handleSubmit = async () => {
    ...
    if (hicistePlanogramaQualid === "Yes") {
      if (!fotoAntesPlanogramaQualid) {
        toast({
          ...
          description:
            'Debe tomar la foto "antes" del planograma Qualid cuando indica que trabajó en él.',
        });
        return;
      }
      if (!fotoDespuesPlanogramaQualid) {
        toast({
          ...
          description:
            'Debe tomar la foto "después" del planograma Qualid cuando indica que trabajó en él.',
        });
        return;
      }
    }
    ...
    if (
      ...
      toast({
        ...
        description:
          "Debe tomar una foto de los exhibidores de caucho Qualid cuando los agrega.",
      });
      return;
    }
    ...
    try {
      ...
      try {
        ...
        console.log(
          "📦 [QUALID] Datos existentes en localStorage:",
          datosExistentes ? "Sí existen" : "No existen"
        );
        ...
        console.error(
          "❌ [QUALID] Error parseando datos existentes:",
          parseError
        );
      ...
      // 🔧 NO guardar visita completa aquí - continuar con el flujo
      // Los datos se guardarán en localStorage y se completarán al final en reportes-finales
      console.log(
        "💾 Datos de Qualid guardados en localStorage (offline-ready)..."
      );
      ...
      console.log(
        "✅ [QUALID] Datos guardados en localStorage exitosamente"
      );
      console.log(
        "📊 [QUALID] Tamaño de datos guardados:",
        datosSerializados.length,
        "caracteres"
      );

      console.log(
        "=== ✅ DATOS DE QUALID MERCHANDISING GUARDADOS EXITOSAMENTE ==="
      );
  ...
  useEffect(() => {
    const syncData = async () => {
      if (isSyncing || typeof window === "undefined" || !window.localStorage)
        return;
      setIsSyncing(true);
      ...
      try {
        ...
        toast({
          ...
          description:
            "Hubo un problema al sincronizar los datos. Inténtalo de nuevo más tarde.",
        });
      } finally {
        setIsSyncing(false);
      }
  ...
  const availableExhibidorCauchoQualidTypes =
    EXHIBIDORES_CAUCHO_QUALID_TYPES.filter(
      ...
      >
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Captura datos de merchandising de Qualid.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <Select
                onValueChange={handleHicistePlanogramaQualidChange}
                value={hicistePlanogramaQualid}
              >
            ...
            {hicistePlanogramaQualid === "Yes" && (
              <>
                <div>
                  <Label htmlFor="foto-antes-planograma-qualid">
                    Foto "Antes" del Planograma Qualid
                  </Label>
                  <Button
                    onClick={() =>
                      takePhoto(
                        setFotoAntesPlanogramaQualid,
                        "fotoAntesPlanogramaQualid"
                      )
                    }
                  ...
                  {fotoAntesPlanogramaQualid && (
                    <img
                      src={fotoAntesPlanogramaQualid}
                      alt="Foto Antes Planograma Qualid"
                ...
                <div>
                  <Label htmlFor="foto-despues-planograma-qualid">
                    Foto "Después" del Planograma Qualid
                  </Label>
                  <Button
                    onClick={() =>
                      takePhoto(
                        setFotoDespuesPlanogramaQualid,
                        "fotoDespuesPlanogramaQualid"
                      )
                    }
                    disabled={!hasCameraPermission || !!capturingType}
                    className="w-full mt-1"
                  ...
                  {fotoDespuesPlanogramaQualid && (
                    <img
                      src={fotoDespuesPlanogramaQualid}
                      alt="Foto Después Planograma Qualid"
                      className="mt-2 rounded-md"
                    />
                  )}
                </div>
            ...
            {/* Cenefas y Bolsas */}
            <div>
              <Label htmlFor="total-cenefas-qualid">
                Total de Cenefas Qualid colocadas
              </Label>
              <Input
                ...
                value={
                  totalCenefasQualid !== null
                    ? totalCenefasQualid.toString()
                    : ""
                }
            ...
            <div>
              <Label htmlFor="bolsas-qualid-carro">
                Bolsas Qualid para carros entregadas {/* <--- AQUÍ */}
              </Label>
              <Input
                ...
                value={
                  bolsasQualidCarro !== null ? bolsasQualidCarro.toString() : ""
                }
            ...
            <div>
              <Label htmlFor="sticker-qualid">
                Sticker de garantia Qualid 
              </Label>
              <Input
                id="sticker-qualid"
                type="number"
                placeholder="Ingresar cantidad"
                value={
                  stickerQualid !== null ? stickerQualid.toString() : ""
                }
                onChange={handleStickerQualidChange}
                inputMode="numeric"
                min="0"
                className="mt-1"
                disabled={isSyncing || !!capturingType}
            ...
            <div className="space-y-4 border-t pt-4">
              ...
              <div className="space-y-3">
                <div className="flex items-end space-x-2">
                  <div className="flex-grow space-y-1">
                    <Label htmlFor="tipo-afiche-qualid-select">
                      Tipo de Afiche Qualid
                    </Label>
                    <Select
                      value={currentTipoAficheQualid}
                      onValueChange={setCurrentTipoAficheQualid}
                      disabled={
                        availableAficheQualidTypes.length === 0 ||
                        isSyncing ||
                        !!capturingType
                      }
                    >
                  ...
                  <div className="w-1/3 space-y-1">
                    <Label htmlFor="cantidad-afiche-qualid-input">
                      Cantidad
                    </Label>
                    <Input
                      ...
                      onChange={(e) =>
                        setCurrentCantidadAficheQualid(e.target.value)
                      }
                      inputMode="numeric"
                      min="0"
                      disabled={isSyncing || !!capturingType}
                ...
                {currentTipoAficheQualid &&
                  currentCantidadAficheQualid &&
                  parseInt(currentCantidadAficheQualid) > 0 && (
                    <div className="space-y-2">
                      <Label htmlFor="foto-afiche-qualid-individual">
                        Foto del {currentTipoAficheQualid}
                      </Label>
                      <Button
                        onClick={() =>
                          takePhoto(
                            setCurrentFotoAficheQualid,
                            "fotoAficheQualidIndividual"
                          )
                        }
                        disabled={!hasCameraPermission || !!capturingType}
                      ...
                      >
                        {capturingType === "fotoAficheQualidIndividual" ? (
                          "Capturando..."
                        ) : hasCameraPermission ? (
                          <>
                            <Camera className="mr-2 h-4 w-4" /> Tomar Foto de
                            este Afiche
                          </>
                        ) : (
                          "Cámara no permitida"
                ...
                <Button
                  ...
                  disabled={
                    ...
                    isSyncing ||
                    !!capturingType ||
                    (parseInt(currentCantidadAficheQualid || "0") > 0 &&
                      !currentFotoAficheQualid)
                  }
                  className="w-full"
                >
                      ...
                      >
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium">
                            {afiche.tipo}: {afiche.cantidad}
                          </span>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleRemoveAficheQualid(index)}
                            disabled={isSyncing || !!capturingType}
                          >
              ...
              {availableAficheQualidTypes.length === 0 &&
                AFICHES_QUALID_TYPES.length > 0 &&
                afichesQualidAgregados.length ===
                  AFICHES_QUALID_TYPES.length && (
                  <p className="text-sm text-muted-foreground text-center mt-2">
                    Todos los tipos de afiches Qualid disponibles han sido
                    agregados.
                  </p>
                )}
            </div>
            ...
            <div className="space-y-4 border-t pt-4">
              <Label className="font-medium">
                Exhibidores de Cauchos Qualid
              </Label>
              <div className="flex items-end space-x-2">
                <div className="flex-grow space-y-1">
                  <Label htmlFor="tipo-exhibidor-caucho-qualid-select">
                    Tipo de Exhibidor de Caucho
                  </Label>
                  <Select
                    value={currentTipoExhibidorCauchoQualid}
                    onValueChange={setCurrentTipoExhibidorCauchoQualid}
                    disabled={
                      availableExhibidorCauchoQualidTypes.length === 0 ||
                      isSyncing ||
                      !!capturingType
                    }
                  >
                ...
                <div className="w-1/3 space-y-1">
                  <Label htmlFor="cantidad-exhibidor-caucho-qualid-input">
                    Cantidad
                  </Label>
                  <Input
                    ...
                    onChange={(e) =>
                      setCurrentCantidadExhibidorCauchoQualid(e.target.value)
                    }
                    inputMode="numeric"
                    min="0"
                    disabled={isSyncing || !!capturingType}
              ...
              {exhibidoresCauchoQualidAgregados.length > 0 && (
                <div className="mt-4 space-y-2">
                  <Label className="text-sm text-muted-foreground">
                    Exhibidores de Cauchos Qualid agregados:
                  </Label>
                  <ul className="space-y-1">
                    {exhibidoresCauchoQualidAgregados.map(
                      (exhibidor, index) => (
                        <li
                          key={index}
                        ...
                        >
                          <span className="text-sm">
                            {exhibidor.tipo}: {exhibidor.cantidad}
                          </span>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() =>
                              handleRemoveExhibidorCauchoQualid(index)
                            }
                            disabled={isSyncing || !!capturingType}
                          >
                            <Trash className="h-4 w-4 text-destructive" />
              ...
              {availableExhibidorCauchoQualidTypes.length === 0 &&
                EXHIBIDORES_CAUCHO_QUALID_TYPES.length > 0 &&
                exhibidoresCauchoQualidAgregados.length ===
                  EXHIBIDORES_CAUCHO_QUALID_TYPES.length && (
                  <p className="text-sm text-muted-foreground text-center mt-2">
                    Todos los tipos de exhibidores de caucho Qualid disponibles
                    han sido agregados.
                  </p>
                )}
            </div>
            ...
            {exhibidoresCauchoQualidAgregados.length > 0 && (
              <div>
                <Label htmlFor="foto-exhibidores-caucho-qualid">
                  Foto de los Exhibidores de Cauchos Qualid
                </Label>
                <Button
                  onClick={() =>
                    takePhoto(
                      setFotoExhibidoresCauchoQualid,
                      "fotoExhibidoresCauchoQualid"
                    )
                  }
                  disabled={
                    !hasCameraPermission || !!capturingType || isSyncing
                  }
                ...
                >
                  {capturingType === "fotoExhibidoresCauchoQualid" ? (
                    "Capturando..."
                  ) : hasCameraPermission ? (
                    <>
                      <Camera className="mr-2 h-4 w-4" /> Tomar Foto de
                      Exhibidores de Cauchos
                    </>
                  ) : (
                    "Cámara no permitida"
                  )}
          ...
          <CardFooter>
            ...
            >
              {isSyncing
                ? "Sincronizando..."
                : "Guardar y Continuar a Observaciones"}
            </Button>
          </CardFooter>
        </Card>
      </div>

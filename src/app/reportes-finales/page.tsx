    ...
    if (typeof window !== "undefined" && !navigator.onLine) {
      try {
        ...
        if (saveResult.success) {
          ...
          const puntoId =
            datosAcumulados.clienteData?.id || datosAcumulados.clienteData?.rif;
          if (puntoId) {
            const puntosCompletados = JSON.parse(
              localStorage.getItem("puntosCompletados") || "[]"
            );
            if (!puntosCompletados.includes(puntoId)) {
              puntosCompletados.push(puntoId);
              localStorage.setItem(
                "puntosCompletados",
                JSON.stringify(puntosCompletados)
              );
            }
          }

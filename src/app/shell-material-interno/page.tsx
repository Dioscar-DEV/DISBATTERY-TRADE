...
export default function ShellMaterialInternoPage() {
  ...
  const [cantidadExhibidores, setCantidadExhibidores] = useState<number | null>(
    null
  );
  const [fotoExhibidoresShell, setFotoExhibidoresShell] = useState<
    string | null
  >(null);
  ...
  const [currentCantidadAfiche, setCurrentCantidadAfiche] =
    useState<string>("");
  const [currentFotoAfiche, setCurrentFotoAfiche] = useState<string | null>(
    null
  );

  const [colocoBanderines, setColocoBanderines] = useState<string>("");
  const [cantidadTirasBanderines, setCantidadTirasBanderines] = useState<
    number | null
  >(null);
  ...
  const [fotoAvisoAcrilico, setFotoAvisoAcrilico] = useState<string | null>(
    null
  );
  ...
  const handleCantidadExhibidoresChange = (
  ...
  const handleRemoveAfiche = (indexToRemove: number) => {
    setAfichesAgregados(
      afichesAgregados.filter((_, index) => index !== indexToRemove)
    );
  };
  ...
  const handleCantidadTirasBanderinesChange = (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    const value = e.target.value;
  ...
  const handleSubmit = async () => {
    ...
    if (tieneExhibidores === "") {
      toast({
        ...
        description:
          "Por favor, indique si el cliente tiene exhibidores Shell.",
      });
      return;
    }
    ...
    if (tieneExhibidores === "Yes" && !fotoExhibidoresShell) {
      toast({
        ...
        description:
          "Debe tomar una foto de los exhibidores Shell cuando indica que el cliente SÍ tiene.",
      });
      return;
    }
    ...
    if (colocoBanderines === "Yes" && !fotoBanderines) {
      toast({
        ...
        description:
          "Debe tomar una foto de los banderines cuando indica que SÍ los colocó.",
      });
      return;
    }
    ...
    if (colocoAvisoAcrilico === "Yes" && !fotoAvisoAcrilico) {
      toast({
        ...
        description:
          "Debe tomar una foto del aviso acrílico cuando indica que SÍ lo colocó.",
      });
      return;
    }
    ...
    try {
      ...
      console.log(
        "💾 Datos de Material Interno guardados en localStorage (offline-ready)..."
      );
      ...
      toast({
        ...
        description:
          "Hubo un problema guardando el progreso. Intente nuevamente.",
      });
    }
  };
  ...
  useEffect(() => {
    const syncData = async () => {
      ...
      try {
        ...
        console.log(
          "Enviando datos de Material Interno Shell al servidor:",
          localData
        );
        ...
        toast({
          ...
          description:
            "Todos los datos de material interno se han enviado al servidor.",
        });
      } catch (error) {
        console.error(
          "Error al sincronizar los datos de material interno Shell:",
          error
        );
        toast({
          variant: "destructive",
        ...
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-center">
              Registre los materiales internos de Shell colocados.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <Select
                onValueChange={handleTieneExhibidoresChange}
                value={tieneExhibidores}
            ...
            {tieneExhibidores === "Yes" && (
              <>
                <div>
                  <Label htmlFor="cantidad-exhibidores">
                    ¿Cuántos exhibidores Shell tiene?
                  </Label>
                  <Input
                    type="number"
                ...
                <div>
                  <Label htmlFor="foto-exhibidores-shell">
                    Foto del Exhibidor Shell
                  </Label>
                  <Button
                    onClick={() =>
                      takePhoto(setFotoExhibidoresShell, "fotoExhibidoresShell")
                    }
                  ...
                  >
                    {capturingType === "fotoExhibidoresShell" ? (
                      "Capturando..."
                    ) : hasCameraPermission ? (
                      <>
            ...
            <div className="space-y-4 border-t pt-4">
              ...
              <div className="space-y-3">
                <div className="flex items-end space-x-2">
                  <div className="flex-grow space-y-1">
                    ...
                    <Select
                      ...
                      disabled={
                        availableAficheTypes.length === 0 || !!capturingType
                      }
                    >
                      <SelectTrigger id="tipo-afiche-select">
                        <SelectValue
                          placeholder={
                            availableAficheTypes.length > 0
                              ? "Seleccionar tipo"
                              : "Todos agregados"
                          }
                        />
                ...
                {currentTipoAfiche &&
                  currentCantidadAfiche &&
                  parseInt(currentCantidadAfiche) > 0 && (
                    <div className="space-y-2">
                      <Label htmlFor="foto-afiche-individual">
                        Foto del {currentTipoAfiche}
                      </Label>
                      <Button
                        onClick={() =>
                          takePhoto(
                            setCurrentFotoAfiche,
                            "fotoAficheIndividual"
                          )
                        }
                        disabled={!hasCameraPermission || !!capturingType}
                      ...
                      >
                        {capturingType === "fotoAficheIndividual" ? (
                          "Capturando..."
                        ) : hasCameraPermission ? (
                          <>
                ...
                <Button
                  ...
                  disabled={
                    ...
                    availableAficheTypes.length === 0 ||
                    !!capturingType ||
                    (parseInt(currentCantidadAfiche || "0") > 0 &&
                      !currentFotoAfiche)
                  }
                  className="w-full"
                >
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleRemoveAfiche(index)}
                            disabled={!!capturingType}
                          >
              ...
              {availableAficheTypes.length === 0 &&
                AFICHES_SHELL_TYPES.length > 0 &&
                afichesAgregados.length === AFICHES_SHELL_TYPES.length && (
                  <p className="text-sm text-muted-foreground text-center mt-2">
                    Todos los tipos de afiches disponibles han sido agregados.
                  </p>
                )}
            </div>
            ...
            <div className="space-y-2 border-t pt-4">
              ...
              <Select
                onValueChange={handleColocoBanderinesChange}
                value={colocoBanderines}
                disabled={!!capturingType}
              >
                <SelectTrigger className="w-full mt-1">
                  <SelectValue placeholder="Seleccionar" />
            ...
            {colocoBanderines === "Yes" && (
              <>
                <div>
                  <Label htmlFor="cantidad-tiras-banderines">
                    Cantidad de tiras de banderines
                  </Label>
                  <Input
                    type="number"
                    ...
                    placeholder="Ingresar cantidad"
                    value={
                      cantidadTirasBanderines !== null
                        ? cantidadTirasBanderines.toString()
                        : ""
                    }
                ...
                <div>
                  <Label htmlFor="foto-banderines">
                    Foto Soporte Banderines
                  </Label>
                  <Button
                    onClick={() =>
                      takePhoto(setFotoBanderines, "fotoBanderines")
                    }
                    disabled={!hasCameraPermission || !!capturingType}
                    className="w-full mt-1 text-white"
                    style={{
            ...
            <div className="space-y-2 border-t pt-4">
              ...
              <Select
                onValueChange={handleColocoAvisoAcrilicoChange}
                value={colocoAvisoAcrilico}
                disabled={!!capturingType}
              >
                <SelectTrigger className="w-full mt-1">
                  <SelectValue placeholder="Seleccionar" />
            ...
            {colocoAvisoAcrilico === "Yes" && (
              <div>
                <Label htmlFor="foto-aviso-acrilico">
                  Foto del Aviso Acrílico
                </Label>
                <Button
                  onClick={() =>
                    takePhoto(setFotoAvisoAcrilico, "fotoAvisoAcrilico")
                  }
                  disabled={!hasCameraPermission || !!capturingType}
                  className="w-full mt-1 text-white"
                  style={{
                ...
                >
                  {capturingType === "fotoAvisoAcrilico" ? (
                    "Capturando..."
                  ) : hasCameraPermission ? (
                    <>
                      <Camera className="mr-2 h-4 w-4" /> Tomar Foto del Aviso
                      Acrílico
                    </>
                  ) : (
                    "Cámara no permitida"

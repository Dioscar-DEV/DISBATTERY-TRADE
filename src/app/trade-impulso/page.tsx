...
export default function TradeImpulsoPage() {
  ...
  useEffect(() => {
    ...
    if (marcaFromRoute) {
      ...
      console.log(
        "🎯 Marca establecida automáticamente desde ruta:",
        marcaFromRoute
      );
    }
  }, []);
      ...
      if (todaysRoutesString) {
        try {
          ...
          if (currentRoute) {
            ...
            console.log(
              "❌ No se encontró ruta para este cliente con Trade (Impulso)"
            );
          }
        } catch (error) {
      ...
      if (visitDataString) {
        try {
          ...
          if (visitData.marcaTrabajada) {
            console.log(
              "✅ MARCA ENCONTRADA EN VISIT DATA:",
  ...
  const [fotoPromotorasShell, setFotoPromotorasShell] = useState<string | null>(
    null
  );
  const [fotoImpulsoQualid, setFotoImpulsoQualid] = useState<string | null>(
    null
  );
  const [fotoPromotorasQualid, setFotoPromotorasQualid] = useState<
    string | null
  >(null);

  // Estados para ventas
  const [huboVentasShell, setHuboVentasShell] = useState<boolean | null>(null);
  const [huboVentasQualid, setHuboVentasQualid] = useState<boolean | null>(
    null
  );
  ...
  const saveDataLocally = (data: any) => {
    if (typeof window !== "undefined" && window.localStorage) {
      const tradeImpulsoData = JSON.parse(
        localStorage.getItem("tradeImpulsoData") || "[]"
      );
      tradeImpulsoData.push(data);
      localStorage.setItem(
        "tradeImpulsoData",
        JSON.stringify(tradeImpulsoData)
      );
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    console.log(
      "🔍 fotoImpulsoShell longitud:",
      fotoImpulsoShell ? fotoImpulsoShell.length : 0
    );
    console.log(
      "🔍 fotoImpulsoShell es base64 válido:",
      fotoImpulsoShell ? fotoImpulsoShell.startsWith("data:image/") : false
    );

    console.log("🔍 fotoPromotorasShell existe:", !!fotoPromotorasShell);
    console.log(
      "🔍 fotoPromotorasShell longitud:",
      fotoPromotorasShell ? fotoPromotorasShell.length : 0
    );
    console.log(
      "🔍 fotoPromotorasShell es base64 válido:",
      fotoPromotorasShell
        ? fotoPromotorasShell.startsWith("data:image/")
        : false
    );

    console.log("🔍 fotoImpulsoQualid existe:", !!fotoImpulsoQualid);
    console.log(
      "🔍 fotoImpulsoQualid longitud:",
      fotoImpulsoQualid ? fotoImpulsoQualid.length : 0
    );
    console.log(
      "🔍 fotoImpulsoQualid es base64 válido:",
      fotoImpulsoQualid ? fotoImpulsoQualid.startsWith("data:image/") : false
    );

    console.log("🔍 fotoPromotorasQualid existe:", !!fotoPromotorasQualid);
    console.log(
      "🔍 fotoPromotorasQualid longitud:",
      fotoPromotorasQualid ? fotoPromotorasQualid.length : 0
    );
    console.log(
      "🔍 fotoPromotorasQualid es base64 válido:",
      fotoPromotorasQualid
        ? fotoPromotorasQualid.startsWith("data:image/")
        : false
    );
    ...
    if (huboVentasShell === null || huboVentasQualid === null) {
      toast({
        ...
        description:
          "Por favor, responda las preguntas sobre ventas de SHELL y QUALID.",
      });
      return;
    }
    ...
    try {
      ...
      const fotoImpulsoFinal =
        selectedBrand === "Shell" ? fotoImpulsoShell : fotoImpulsoQualid;
      const fotoPromotorasFinal =
        selectedBrand === "Shell" ? fotoPromotorasShell : fotoPromotorasQualid;

      console.log("🔍 MAPEO DE FOTOS FINALES:");
      console.log("🔍 fotoImpulsoFinal existe:", !!fotoImpulsoFinal);
      console.log(
        "🔍 fotoImpulsoFinal longitud:",
        fotoImpulsoFinal ? fotoImpulsoFinal.length : 0
      );
      console.log(
        "🔍 fotoImpulsoFinal es base64 válido:",
        fotoImpulsoFinal ? fotoImpulsoFinal.startsWith("data:image/") : false
      );

      console.log("🔍 fotoPromotorasFinal existe:", !!fotoPromotorasFinal);
      console.log(
        "🔍 fotoPromotorasFinal longitud:",
        fotoPromotorasFinal ? fotoPromotorasFinal.length : 0
      );
      console.log(
        "🔍 fotoPromotorasFinal es base64 válido:",
        fotoPromotorasFinal
          ? fotoPromotorasFinal.startsWith("data:image/")
          : false
      );

      // Preparar datos para guardar localmente (sin enviar aún)
      const tradeImpulsoData = {
      ...
      console.log(
        "🔍 tradeImpulsoData.fotoImpulso existe:",
        !!tradeImpulsoData.fotoImpulso
      );
      console.log(
        "🔍 tradeImpulsoData.fotoPromotoras existe:",
        !!tradeImpulsoData.fotoPromotoras
      );
      console.log(
        "🔍 tradeImpulsoData.fotoImpulsoShell existe:",
        !!tradeImpulsoData.fotoImpulsoShell
      );
      console.log(
        "🔍 tradeImpulsoData.fotoPromotorasShell existe:",
        !!tradeImpulsoData.fotoPromotorasShell
      );
      console.log(
        "🔍 tradeImpulsoData.fotoImpulsoQualid existe:",
        !!tradeImpulsoData.fotoImpulsoQualid
      );
      console.log(
        "🔍 tradeImpulsoData.fotoPromotorasQualid existe:",
        !!tradeImpulsoData.fotoPromotorasQualid
      );
    ...
    const syncData = async () => {
      ...
      try {
        ...
        toast({
          ...
          description:
            "Todos los datos de impulso trade se han enviado al servidor.",
        });
      } catch (error) {
        console.error(
          "Error al sincronizar los datos de Impulso Trade:",
          error
        );
        toast({
          variant: "destructive",
      ...
      >
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Ingrese los detalles del impulso realizado.
            </CardDescription>
          </CardHeader>
            ...
            <div>
              ...
              >
                <SelectTrigger id="marca-select" className="w-full mt-1">
                  <SelectValue
                    placeholder={selectedBrand || "Marca no asignada"}
                  />
                </SelectTrigger>
                <SelectContent>
              ...
              {!selectedBrand && (
                <p className="text-sm text-orange-600 mt-1">
                  ⚠️ Marca no asignada. Verifique que el evento tenga marca
                  trabajada configurada.
                </p>
              )}
            </div>
            ...
            {selectedBrand && (
              <div className="space-y-4 border-t pt-4">
                <Label className="font-medium">
                  Recursos Utilizados ({selectedBrand})
                </Label>
                <div className="flex items-end space-x-2">
                  ...
                  <div className="w-1/3 space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCurrentCantidadRecurso(e.target.value)
                      }
                      inputMode="numeric"
                      min="1"
                      disabled={isSyncing}
                ...
                {availableRecursoTypes.length === 0 &&
                  ...
                  recursosAgregados.length ===
                    ...
                    <p className="text-sm text-muted-foreground text-center mt-2">
                      Todos los tipos de recursos {selectedBrand} disponibles
                      han sido agregados.
                    </p>
                  )}
              </div>
              ...
              <>
                <div className="space-y-4 border-t pt-4">
                  ...
                  {availableEntregableShellTypes.length === 0 &&
                    ...
                    entregablesShellAgregados.length ===
                      ...
                      <p className="text-sm text-muted-foreground text-center mt-2">
                        Todos los tipos de entregables Shell disponibles han
                        sido agregados.
                      </p>
                    )}
                </div>
                ...
                <div className="space-y-4 border-t pt-4">
                  <Label
                    ...
                    htmlFor="foto-impulso-shell-button"
                  >
                    Fotos del impulso Shell
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    Sube fotos generales del evento Shell, incluyendo la
                    instalación del material de apoyo, vista del stand, y las
                    promotoras en el lugar. Asegúrate de capturar la esencia y
                    el ambiente general del impulso o evento Shell.
                  </p>
                  <Button
                    id="foto-impulso-shell-button"
                    variant="outline"
                    onClick={() =>
                      uploadImage(setFotoImpulsoShell, "fotoImpulsoShell")
                    }
                    disabled={isSyncing}
                ...
                <div className="space-y-4 border-t pt-4">
                  <Label
                    className="font-medium"
                    htmlFor="foto-promotoras-shell-button"
                  >
                    Fotos de las promotoras Shell
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    Sube fotos de las promotoras Shell interactuando con los
                    clientes, mostrando momentos de compras, participación en
                    juegos o dinámicas. Buscamos capturar la conexión y el
                    impacto directo del evento Shell en el público.
                  </p>
                  <Button
                    id="foto-promotoras-shell-button"
                    variant="outline"
                    onClick={() =>
                      uploadImage(setFotoPromotorasShell, "fotoPromotorasShell")
                    }
                    disabled={isSyncing}
                    className="w-full mt-1"
                  >
            ...
            {selectedBrand === "Qualid" && (
              <>
                <div className="space-y-4 border-t pt-4">
                        ...
                        {entregablesQualidAgregados.map((entregable, index) => (
                          ...
                          >
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleRemoveEntregableQualid(index)
                              }
                              disabled={isSyncing}
                            >
                              <Trash className="h-4 w-4 text-destructive" />
                  ...
                  {availableEntregableQualidTypes.length === 0 &&
                    ...
                    entregablesQualidAgregados.length ===
                      ...
                      <p className="text-sm text-muted-foreground text-center mt-2">
                        Todos los tipos de entregables Qualid disponibles han
                        sido agregados.
                      </p>
                    )}
                </div>
                ...
                <div className="space-y-4 border-t pt-4">
                  <Label
                    className="font-medium"
                    htmlFor="foto-impulso-qualid-button"
                  >
                    Fotos del impulso Qualid
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    Sube fotos generales del evento Qualid, incluyendo la
                    instalación del material de apoyo, vista del stand, y las
                    promotoras en el lugar. Asegúrate de capturar la esencia y
                    el ambiente general del impulso o evento Qualid.
                  </p>
                  <Button
                    id="foto-impulso-qualid-button"
                    variant="outline"
                    onClick={() =>
                      uploadImage(setFotoImpulsoQualid, "fotoImpulsoQualid")
                    }
                    disabled={isSyncing}
                    className="w-full mt-1"
                  >
                ...
                <div className="space-y-4 border-t pt-4">
                  <Label
                    className="font-medium"
                    htmlFor="foto-promotoras-qualid-button"
                  >
                    Fotos de las promotoras Qualid
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    Sube fotos de las promotoras Qualid interactuando con los
                    clientes, mostrando momentos de compras, participación en
                    juegos o dinámicas. Buscamos capturar la conexión y el
                    impacto directo del evento Qualid en el público.
                  </p>
                  <Button
            ...
            {((selectedBrand === "Shell" &&
              ...
              <>
                <div className="space-y-4 border-t pt-6">
                  <div className="space-y-4">
                    <Label className="text-lg font-semibold">
                      ¿Se reportó venta de productos SHELL?
                    </Label>
                ...
                <div className="space-y-4 border-t pt-6">
                  <div className="space-y-4">
                    <Label className="text-lg font-semibold">
                      ¿Se reportó venta de productos QUALID?
                    </Label>
                    <div className="flex space-x-4">
                      <label className="flex items-center space-x-2">
                        <input

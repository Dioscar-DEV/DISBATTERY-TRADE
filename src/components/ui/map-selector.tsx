...
export function MapSelector({ onPositionSelect, initialPosition, className }: MapSelectorProps) {
  ...
  const predefinedLocations = [
    ...
    { name: 'Barquisimeto', lat: 10.0670, lng: -69.3467 },
    { name: 'Maracay', lat: 10.2469, lng: -67.5958 },
    { name: 'Puerto La Cruz', lat: 10.2138, lng: -64.6328 },
    { name: 'Ciudad Guayana', lat: 8.3536, lng: -62.6410 },
  ...
  const handleManualCoordinates = () => {
    const coords = searchAddress.split(',').map(coord => parseFloat(coord.trim()));
    if (coords.length === 2 && !isNaN(coords[0]) && !isNaN(coords[1])) {
      const position = { lat: coords[0], lng: coords[1] };
      setSelectedPosition(position);
      onPositionSelect(position);
    } else {
      alert('Por favor ingresa las coordenadas en formato: latitud, longitud (ej: 10.4806, -66.9036)');
    }
  };
          ...
          <div className="grid grid-cols-2 gap-2 mt-2">
            {predefinedLocations.map((location) => (
              <Button
                ...
                className={`text-xs ${
                  selectedPosition?.lat === location.lat && selectedPosition?.lng === location.lng
        ...
        {selectedPosition && (
          <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
            ...
            <p className="text-xs text-green-600 mt-1">
              💡 Puedes hacer clic en otra parte del mapa para cambiar la ubicación
            </p>
          </div>
        )}
      </CardContent>

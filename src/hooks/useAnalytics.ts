...
export function useAnalytics() {
  ...
  const trackNavigation = useCallback(async (destination: string, context?: string) => {
  ...
  const trackButtonClick = useCallback(async (buttonName: string, context?: string) => {
  ...
  const trackClientInteraction = useCallback(async (
    clientId: string,
    clientName: string,
  ...
  const trackRouteActivity = useCallback(async (
    routeId: string,
  ...
  return {
    ...
    trackPhotoCapture: analyticsService.trackPhotoCapture.bind(analyticsService),
    trackReportGeneration: analyticsService.trackReportGeneration.bind(analyticsService),
    trackOfflineAction: analyticsService.trackOfflineAction.bind(analyticsService),
...
export function usePageTimer(pageName: string) {
  useEffect(() => {
    ...
    return () => {
      ...
      if (timeSpent > 3) { // Solo trackear si estuvo más de 3 segundos
...
export function useErrorTracking(componentName: string) {
  const trackError = useCallback((error: Error, context?: string) => {
    analyticsService.trackError(error, `${componentName}${context ? `_${context}` : ''}`);
  }, [componentName]);

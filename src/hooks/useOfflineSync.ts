...
export function useOfflineSync() {
    ...
    try {
      ...
      if (success) {
        ...
        serviceWorkerManager.onMessage('sync-complete', (data) => {
          console.log('✅ [useOfflineSync] Sincronización completada desde SW:', data);
          updateSyncStatus();
        });
        
        console.log('✅ [useOfflineSync] Service Worker configurado exitosamente');
      } else {
        console.warn('⚠️ [useOfflineSync] Service Worker no pudo ser inicializado');
      }
    } catch (error) {
      console.error('❌ [useOfflineSync] Error inicializando Service Worker:', error);
    }
  }, [updateSyncStatus]);
  ...
  const triggerSync = useCallback(async (): Promise<SyncResult> => {
    ...
    try {
      ...
      console.log(`✅ [useOfflineSync] Sincronización completada: ${result.processed} procesadas, ${result.errors} errores`);
  ...
  const scheduleAutoSync = useCallback(() => {
    ...
    syncIntervalRef.current = setInterval(async () => {
      if (state.isOnline && !state.isSyncing && state.pendingVisitas > 0) {
        console.log('⏰ [useOfflineSync] Ejecutando sincronización automática...');
        
        // Evitar sincronización si se hizo una reciente (menos de 2 minutos)
        const now = new Date();
        if (lastSyncAttemptRef.current) {
          const timeSinceLastSync = now.getTime() - lastSyncAttemptRef.current.getTime();
          if (timeSinceLastSync < 2 * 60 * 1000) { // 2 minutos
            console.log('⏭️ [useOfflineSync] Sincronización reciente, omitiendo...');
            return;
          }
        }
  ...
  const handleConnectionChange = useCallback(async () => {
    ...
    if (isOnline && state.pendingVisitas > 0) {
      console.log('🌐 [useOfflineSync] Conexión restaurada, iniciando sincronización...');
  ...
  const forceSyncThroughSW = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      console.error('❌ [useOfflineSync] Error forzando sync a través de SW:', error);
      return false;
    }

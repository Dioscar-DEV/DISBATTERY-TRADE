...
export function useOfflineSync() {
  ...
  useEffect(() => {
    const handleOnline = async () => {
      if (navigator.onLine) {
        try {
          // Intentar sincronizar automáticamente al reconectar
          await offlineManager.syncPendingVisitas();
          await updateSyncStatus();
          console.log('✅ Sincronización automática al reconectar ejecutada');
        } catch (error) {
          console.error('❌ Error en sincronización automática al reconectar:', error);
        }
      }
    };

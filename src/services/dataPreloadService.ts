...
class DataPreloadService {
  ...
  private reportProgress(step: string, current: number, total: number, message: string): void {
    ...
    const progress: PreloadProgress = { step, current, total, percentage, message };
    
    console.log(`📊 [DataPreload] ${step}: ${current}/${total} (${percentage}%) - ${message}`);
  ...
  async preloadDataForMercaderista(user: UserData): Promise<PreloadResult> {
    ...
    try {
      console.log(`🚀 [DataPreload] Iniciando precarga para mercaderista: ${user.fullName}`);
      
      // Paso 1: Inicializar base de datos offline
      this.reportProgress('init', 0, 4, 'Inicializando base de datos offline...');
      try {
        await offlineService.initDB();
        console.log('✅ [DataPreload] IndexedDB inicializada correctamente');
      } catch (dbError) {
        console.error('❌ [DataPreload] Error inicializando IndexedDB:', dbError);
      ...
      try {
        ...
        console.error('⚠️ [DataPreload] Error cargando rutas (continuando con array vacío):', routesError);
        routes = [];
      }
      
      // Paso 3: Cargar clientes de las rutas (solo si hay rutas)
      this.reportProgress('clients', 2, 4, 'Descargando información de clientes...');
      if (routes.length > 0) {
        try {
          clientes = await this.loadClientesFromRoutes(routes, user);
          console.log(`✅ [DataPreload] ${clientes.length} clientes obtenidos`);
        } catch (clientesError) {
          console.error('⚠️ [DataPreload] Error cargando clientes (continuando con array vacío):', clientesError);
          clientes = [];
        }
      } else {
        console.log('ℹ️ [DataPreload] Sin rutas disponibles, omitiendo carga de clientes');
        clientes = [];
      }
      ...
      try {
        ...
        console.error('❌ [DataPreload] Error almacenando datos:', storageError);
      ...
      console.log(`📊 [DataPreload] Estadísticas: ${routes.length} rutas, ${clientes.length} clientes, ${totalSizeMB.toFixed(2)}MB`);
      ...
      console.error('❌ [DataPreload] Error crítico durante la precarga:', error);
      
      // ✅ SIEMPRE REPORTAR ERROR FINAL PARA NO DEJAR LA UI COLGADA
      this.reportProgress('complete', 4, 4, 'Error en precarga - Continuando con datos locales');
      ...
      try {
        ...
        console.error('❌ [DataPreload] Error final almacenando datos parciales:', finalError);
      }
      ...
      return {
        ...
        duration,
        error: error instanceof Error ? error.message : 'Error desconocido en precarga'
      };
    }
  }
  ...
  private async loadMercaderistaRoutes(user: UserData): Promise<Route[]> {
    try {
      ...
      console.log(`📅 [DataPreload] Buscando rutas del ${startDateStr} al ${endDateStr}`);
      ...
      try {
        ...
        console.log(`🔍 [DataPreload] Cargando rutas por mercaderistoId: ${user.uid}`);
        const mercaderistaQuery = query(routesRef, where('mercaderistoId', '==', user.uid));
        ...
        console.log(`📱 [DataPreload] ${routes.length} rutas encontradas por mercaderistoId`);

      } catch (mercaderistaError) {
        console.error('⚠️ [DataPreload] Error cargando rutas por mercaderistoId:', mercaderistaError);
      ...
      if (user.sede) {
        try {
          ...
          sedeSnapshot.forEach(doc => {
            ...
            if (routeDate >= startDateStr && routeDate <= endDateStr && !routes.find(r => r.id === doc.id)) {
              routes.push({
                id: doc.id,
                ...routeData,
                createdAt: routeData.createdAt?.toDate ? routeData.createdAt.toDate() : new Date(routeData.createdAt)
          ...
          console.log(`📱 [DataPreload] ${routes.length} rutas totales después de agregar por sede`);

        } catch (sedeError) {
          console.error('⚠️ [DataPreload] Error cargando rutas por sede:', sedeError);
      ...
      if (routes.length === 0 && user.region) {
        try {
          console.log(`🔍 [DataPreload] Fallback: Cargando rutas por región: ${user.region}`);
          const regionQuery = query(routesRef, where('region', '==', user.region));
          ...
          console.log(`📱 [DataPreload] ${routes.length} rutas totales después de fallback por región`);

        } catch (regionError) {
          console.error('⚠️ [DataPreload] Error en fallback por región:', regionError);
        }
      }

      console.log(`✅ [DataPreload] ${routes.length} rutas cargadas exitosamente para ${user.fullName}`);
      return routes;
   ...
   */
  private async loadClientesFromRoutes(routes: Route[], user: UserData): Promise<Cliente[]> {
    const clientes: Cliente[] = [];
    
    try {
      // Validar que hay rutas para procesar
      if (!routes || routes.length === 0) {
        console.log('ℹ️ [DataPreload] No hay rutas disponibles para extraer clientes');
        return [];
      }
      ...
      if (clienteIds.size === 0) {
        console.log('ℹ️ [DataPreload] No se encontraron IDs de clientes en las rutas');
        return [];
      }

      console.log(`👥 [DataPreload] Extrayendo ${clienteIds.size} clientes únicos de las rutas...`);
      ...
      for (let i = 0; i < clienteIdsArray.length; i += batchSize) {
        try {
          ...
          console.log(`👥 [DataPreload] Procesando lote ${Math.floor(i/batchSize) + 1}: IDs ${batch.join(', ')}`);
          ...
          querySnapshot.forEach(doc => {
            try {
              ...
              clientes.push({
                id: doc.id,
                ...clienteData,
                createdAt: clienteData.createdAt?.toDate ? clienteData.createdAt.toDate() : new Date(clienteData.createdAt || Date.now()),
                updatedAt: clienteData.updatedAt?.toDate ? clienteData.updatedAt.toDate() : new Date(clienteData.updatedAt || Date.now())
              } as Cliente);
            } catch (docError) {
              console.error(`⚠️ [DataPreload] Error procesando documento cliente ${doc.id}:`, docError);
              // Continuar con otros documentos
            }
          });

          // Reportar progreso por lotes
          this.reportProgress('clients', Math.min(i + batchSize, clienteIdsArray.length), clienteIdsArray.length, 
            `Clientes: ${clientes.length}/${clienteIdsArray.length} descargados`);

        } catch (batchError) {
          console.error(`⚠️ [DataPreload] Error cargando lote de clientes (${i}-${i+batchSize}):`, batchError);
      ...
      if (user.sede || user.region) {
        try {
          console.log(`👥 [DataPreload] Cargando clientes adicionales por ${user.sede ? 'sede' : 'región'}...`);
          ...
          additionalSnapshot.forEach(doc => {
            try {
              ...
              console.error(`⚠️ [DataPreload] Error procesando cliente adicional ${doc.id}:`, additionalDocError);
              // Continuar con otros documentos
            }
          });

          console.log(`👥 [DataPreload] ${additionalCount} clientes adicionales agregados por ${user.sede ? 'sede' : 'región'}`);

        } catch (additionalError) {
          console.error('⚠️ [DataPreload] Error cargando clientes adicionales por sede/región:', additionalError);
          // Continuar sin clientes adicionales
        }
      }

      console.log(`✅ [DataPreload] ${clientes.length} clientes cargados exitosamente de ${clienteIdsArray.length} solicitados`);
      return clientes;
      ...
      console.log(`⚠️ [DataPreload] Retornando ${clientes.length} clientes parciales debido al error`);
      return clientes;
    }
  }
  ...
  async cleanupOldOfflineData(): Promise<void> {
    try {
      ...
      console.log(`🧹 [DataPreload] Limpieza de datos antiguos (anterior a ${new Date(thirtyDaysAgo).toISOString()})`);

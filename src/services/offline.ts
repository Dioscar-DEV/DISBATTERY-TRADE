 ...
 */
export const solicitarAlmacenamientoPersistente =
  async (): Promise<boolean> => {
    if (navigator.storage && navigator.storage.persist) {
      try {
        ...
        console.log(
          "🔄 Solicitando permiso para almacenamiento persistente..."
        );
        ...
        if (resultado) {
          ...
          console.warn(
            "⚠️ El permiso de almacenamiento persistente no fue concedido."
          );
        }
        return resultado;
      } catch (error) {
        console.error(
          "❌ Error al solicitar almacenamiento persistente:",
          error
        );
        return false;
      }
    ...
    console.warn(
      "⚠️ La API de Almacenamiento Persistente no está soportada en este navegador."
    );
    return false;
  };

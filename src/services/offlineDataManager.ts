...
class OfflineDataManager {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      if (success) {
        ...
        console.log('✅ [OfflineDataManager] Sistema offline inicializado correctamente');
        return true;
      }
  ...
  async shouldDownloadData(user: UserData): Promise<{
    ...
    try {
      ...
      const latestRoute = routes.reduce((latest, route) => 
        route.lastSyncAt > latest ? route.lastSyncAt : latest, 0
      );
  ...
  async forceDownloadData(
    ...
    try {
      ...
      onProgress?.({ step: 'init', percentage: 10, message: 'Preparando descarga...' });
      ...
      onProgress?.({ step: 'download', percentage: 50, message: 'Descargando rutas y clientes...' });
      ...
      onProgress?.({ step: 'complete', percentage: 100, message: 'Descarga completada' });
      
      if (result.success) {
        console.log('✅ [OfflineDataManager] Datos descargados correctamente');
        return { success: true };
      } else {
        console.error('❌ [OfflineDataManager] Error en descarga:', result.error);
  ...
  async downloadDataIfNeeded(
    ...
    try {
      ...
      if (!check.needsDownload) {
        console.log(`ℹ️ [OfflineDataManager] No es necesario descargar: ${check.reason}`);
        return { success: true, downloaded: false };
      }
      
      console.log(`⬇️ [OfflineDataManager] Iniciando descarga: ${check.reason}`);
      ...
      console.error('❌ [OfflineDataManager] Error en descarga condicional:', error);
      return { 
        success: false, 
        downloaded: false,
        error: error instanceof Error ? error.message : 'Error desconocido' 
  ...
  async getDataStats(user: UserData): Promise<{
    ...
    try {
      ...
      const lastSync = latestRoute.length > 0 

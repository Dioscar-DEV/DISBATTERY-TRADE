...
class OfflineManager {
  ...
  async saveVisita(visitaData: any): Promise<SaveResult> {
    try {
      ...
      if (isOnline) {
        ...
        console.log('🌐 [OfflineManager] Conexión disponible - guardando online');
  ...
  private async saveOffline(visitaData: any): Promise<SaveResult> {
    try {
      ...
      const offlineVisita: OfflineVisitaData = {
        id: visitaId,
        clienteData: visitaData.clienteData,
        formData: visitaData,
        fotos: this.extractPhotos(visitaData),
        tipoVisita: visitaData.tipoVisita,
        mercaderistoId: visitaData.mercaderistoId || visitaData.clienteData?.mercaderistoId,
        timestamp: Date.now(),
        gpsLocation: visitaData.gpsCoordinates || visitaData.location,
        status: 'pending',
        retryCount: 0
      };
      ...
      try {
        ...
        console.log('✅ [OfflineManager] Visita guardada en IndexedDB:', visitaId);
      } catch (dbError) {
        console.warn('⚠️ [OfflineManager] IndexedDB falló, usando fallback storage:', dbError);
        
        // Usar fallback storage (localStorage)
        if (fallbackStorage.isAvailable()) {
          await fallbackStorage.saveVisita(visitaData);
          console.log('✅ [OfflineManager] Visita guardada en fallback storage:', visitaId);
        } else {
          throw new Error('No hay almacenamiento disponible (IndexedDB y localStorage fallaron)');
        }
      }
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Error guardando offline'
      };
    }
  }
  ...
  private async saveOnline(visitaData: any): Promise<SaveResult> {
    try {
      ...
      const firebaseData = await this.prepareFirebaseData(visitaData, fotosUrls);
  ...
  async syncPendingVisitas(): Promise<void> {
    ...
    const isOnline = await this.checkConnection();
    if (!isOnline) {
      console.log('📱 [OfflineManager] Sin conexión - sincronización pospuesta');
      return;
    }

    this.syncInProgress = true;
    console.log('🚀 [OfflineManager] Iniciando sincronización de visitas pendientes...');
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ [OfflineManager] Error accediendo a IndexedDB:', dbError);
      }
      ...
      const { uniqueIndexedDB, uniqueFallback, duplicatesRemoved } = this.deduplicateVisitas(pendingVisitas, fallbackVisitas);
      
      if (duplicatesRemoved > 0) {
        console.log(`🔄 [OfflineManager] ${duplicatesRemoved} duplicados eliminados durante deduplicación`);
      ...
      console.log(`📊 [OfflineManager] ${totalPending} visitas pendientes encontradas (${pendingVisitas.length} IndexedDB, ${fallbackVisitas.length} fallback)`);
      ...
      for (const visita of pendingVisitas) {
        try {
          ...
          if (this.isAdminData(visita.data)) {
            ...
            const firebaseData = await this.prepareFirebaseData(visita.data, fotosUrls);
            const visitaId = await crearVisita(firebaseData);
          }
          ...
          progress.processed++;
          console.log(`✅ [OfflineManager] Datos sincronizados: ${visita.visitaId}`);

        } catch (error) {
          console.error(`❌ [OfflineManager] Error sincronizando visita ${visita.visitaId}:`, error);
      ...
      for (const fallbackVisita of fallbackVisitas) {
        try {
          ...
          if (this.isAdminData(fallbackVisita.data)) {
            ...
            // Procesar visitas normales (merchandising)
            const fotosUrls = await this.processAndUploadImages(fallbackVisita.data);
            const firebaseData = await this.prepareFirebaseData(fallbackVisita.data, fotosUrls);
            const visitaId = await crearVisita(firebaseData);
          }
          ...
          progress.processed++;
          console.log(`✅ [OfflineManager] Datos fallback sincronizados: ${fallbackVisita.id}`);

        } catch (error) {
          console.error(`❌ [OfflineManager] Error sincronizando visita fallback ${fallbackVisita.id}:`, error);
          progress.errors++;
        }
      ...
      console.log(`📊 [OfflineManager] Sincronización completada: ${progress.processed} exitosas, ${progress.errors} errores`);

    } catch (error) {
      console.error('❌ [OfflineManager] Error general en sincronización:', error);
    } finally {
   ...
   */
  private validateVisitaData(visitaData: any): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  private extractPhotos(visitaData: any): Record<string, string> {
    ...
    console.log('  - fotoImpulso:', visitaData.fotoImpulso ? 'EXISTE' : 'NO EXISTE');
    console.log('  - fotoPromotoras:', visitaData.fotoPromotoras ? 'EXISTE' : 'NO EXISTE');
    console.log('  - fotoImpulsoShell:', visitaData.fotoImpulsoShell ? 'EXISTE' : 'NO EXISTE');
    console.log('  - fotoPromotorasShell:', visitaData.fotoPromotorasShell ? 'EXISTE' : 'NO EXISTE');
    console.log('  - fotoImpulsoQualid:', visitaData.fotoImpulsoQualid ? 'EXISTE' : 'NO EXISTE');
    console.log('  - fotoPromotorasQualid:', visitaData.fotoPromotorasQualid ? 'EXISTE' : 'NO EXISTE');

    // ✅ NUEVO: Extracción de fotos de evento por marca
    if (visitaData.fotosShell && Array.isArray(visitaData.fotosShell)) {
      console.log(`📸 Extrayendo ${visitaData.fotosShell.length} fotos de Shell...`);
    ...
    if (visitaData.fotosQualid && Array.isArray(visitaData.fotosQualid)) {
      console.log(`📸 Extrayendo ${visitaData.fotosQualid.length} fotos de Qualid...`);
    ...
    if (visitaData.fotosEvento && Array.isArray(visitaData.fotosEvento)) {
      console.log(`📸 Extrayendo ${visitaData.fotosEvento.length} fotos de evento...`);
    ...
    console.log('🚩 visitaData.signagePhoto existe:', !!visitaData.signagePhoto);
    console.log('🚩 visitaData.clienteData?.signagePhoto existe:', !!visitaData.clienteData?.signagePhoto);

    const fotoSeñalizacion = visitaData.signagePhoto || visitaData.clienteData?.signagePhoto;
    ...
    if (visitaData.fotoImpulso && visitaData.fotoImpulso.trim() !== '' && visitaData.fotoImpulso.startsWith('data:image/')) {
      fotos.foto_impulso = visitaData.fotoImpulso;
    } else {
      // Recuperar fotos específicas por marca
      if (visitaData.fotoImpulsoShell && visitaData.fotoImpulsoShell.trim() !== '' && visitaData.fotoImpulsoShell.startsWith('data:image/')) {
        fotos.foto_impulso_shell = visitaData.fotoImpulsoShell;
      }
      if (visitaData.fotoImpulsoQualid && visitaData.fotoImpulsoQualid.trim() !== '' && visitaData.fotoImpulsoQualid.startsWith('data:image/')) {
        fotos.foto_impulso_qualid = visitaData.fotoImpulsoQualid;
      }
    }

    if (visitaData.fotoPromotoras && visitaData.fotoPromotoras.trim() !== '' && visitaData.fotoPromotoras.startsWith('data:image/')) {
      fotos.foto_promotoras = visitaData.fotoPromotoras;
    } else {
      // Recuperar fotos específicas por marca
      if (visitaData.fotoPromotorasShell && visitaData.fotoPromotorasShell.trim() !== '' && visitaData.fotoPromotorasShell.startsWith('data:image/')) {
        fotos.foto_promotoras_shell = visitaData.fotoPromotorasShell;
      }
      if (visitaData.fotoPromotorasQualid && visitaData.fotoPromotorasQualid.trim() !== '' && visitaData.fotoPromotorasQualid.startsWith('data:image/')) {
    ...
    if (visitaData.qualidMerchandising) {
      ...
      if (qualid.fotoDespuesPlanogramaQualid && qualid.fotoDespuesPlanogramaQualid.trim() !== '' && qualid.fotoDespuesPlanogramaQualid.startsWith('data:image/')) {
        fotos.foto_despues_planograma_qualid = qualid.fotoDespuesPlanogramaQualid;
      }
      ...
      if (qualid.fotoExhibidoresCauchoQualid && qualid.fotoExhibidoresCauchoQualid.trim() !== '' && qualid.fotoExhibidoresCauchoQualid.startsWith('data:image/')) {
        fotos.foto_exhibidores_caucho_qualid = qualid.fotoExhibidoresCauchoQualid;
      }
    }
    ...
    const findBase64Images = (obj: any, prefix = '') => {
      for (const [key, value] of Object.entries(obj)) {
        if (typeof value === 'string' && value.startsWith('data:image/')) {
          ...
          if (!fotos[fotoKey]) { // Solo agregar si no existe ya
            fotos[fotoKey] = value;
          }
    ...
    console.log(`📊 [OfflineManager] TOTAL DE FOTOS EXTRAÍDAS: ${Object.keys(fotos).length} imágenes`);
  ...
  private async processAndUploadImages(visitaData: any): Promise<string[]> {
    ...
    console.log(`📸 [OfflineManager] Procesando ${fotosArray.length} imágenes...`);
   ...
   */
  private async prepareFirebaseData(visitaData: any, fotosUrls: string[]): Promise<any> {
    ...
    if (visitaData.huboVentasShell === true && visitaData.ventasShellDetalladas) {
    ...
    if (visitaData.huboVentasQualid === true && visitaData.ventasQualidDetalladas) {
    ...
    const datosSheet = this.prepareCompleteSheetData(visitaData, currentUser, ventasData);
    ...
    return {
      ...
      correoMercaderista: currentUser?.email || visitaData.correoMercaderista || '',
      ubicacion: visitaData.gpsCoordinates || visitaData.location || {
        lat: 0,
        lng: 0,
   ...
   * 📊 Preparar datos COMPLETOS para Google Sheets (FUNCIONALIDAD CRÍTICA RESTAURADA)
   */
  private prepareCompleteSheetData(visitaData: any, currentUser: any, ventasData: string[]): Record<string, any> {
    const cliente = visitaData.clienteData;
    ...
    // INFORMACIÓN BÁSICA
    observacionesOrganizadas.push(`TIPO DE VISITA: ${visitaData.tipoVisita || 'No especificado'}`);
    observacionesOrganizadas.push(`MARCA SELECCIONADA: ${visitaData.marca || 'No especificada'}`);
    observacionesOrganizadas.push(`MERCADERISTA: ${currentUser?.fullName || visitaData.mercaderista || 'Usuario App'}`);
    ...
    if (visitaData.hasSignage) {
      observacionesOrganizadas.push(`¿EL CLIENTE TIENE SEÑALIZACIÓN?: ${visitaData.hasSignage === 'Yes' ? 'Sí' : 'No'}`);
    }
    ...
    if (visitaData.reporteShellFaltante) {
      observacionesOrganizadas.push('');
      observacionesOrganizadas.push('=== REPORTE PRODUCTOS SHELL FALTANTES ===');
      observacionesOrganizadas.push(visitaData.reporteShellFaltante);
    }

    if (visitaData.reporteQualidFaltante) {
      observacionesOrganizadas.push('');
      observacionesOrganizadas.push('=== REPORTE PRODUCTOS QUALID FALTANTES ===');
    ...
    return {
      ...
      'Dirección de correo electrónico': currentUser?.email || visitaData.correoMercaderista || '',
   ...
   */
  private prepareSheetData(visitaData: any, currentUser: any): Record<string, any> {
    const cliente = visitaData.clienteData;
  ...
  private async markPointAsCompleted(visitaData: any): Promise<void> {
    try {
      ...
      if (!pointId) {
        console.warn('⚠️ [OfflineManager] No se encontró ID de punto para marcar como completado');
        return;
      }
      ...
      console.log(`✅ [OfflineManager] Punto ${pointId} marcado como completado`);
    } catch (error) {
      console.error('❌ [OfflineManager] Error marcando punto como completado:', error);
    }
  }
  ...
  private updateLocalStorageRoutes(pointId: string): void {
    try {
      ...
      console.error('❌ [OfflineManager] Error actualizando localStorage:', error);
    }
  }
  ...
  private scheduleSync(): void {
    ...
    if ('serviceWorker' in navigator && 'SyncManager' in window) {
      navigator.serviceWorker.ready
        ...
        .catch((error) => {
          console.error('❌ [OfflineManager] Error registrando Background Sync:', error);
        });
    }

    // Listener para cuando se recupere la conexión
    window.addEventListener('online', () => {
      console.log('🌐 [OfflineManager] Conexión recuperada - iniciando sincronización');
  ...
  private notifyProgress(progress: SyncProgress): void {
    this.progressCallbacks.forEach(callback => {
      try {
        ...
        console.error('❌ [OfflineManager] Error en callback de progreso:', error);
      }
    });
  }
  ...
  async getSyncStats(): Promise<{
    ...
    try {
      ...
      try {
        ...
        stats.pending += allVisitas.filter((v: any) => v.syncStatus === 'pending').length;
        stats.synced += allVisitas.filter((v: any) => v.syncStatus === 'synced').length;
        stats.errors += allVisitas.filter((v: any) => v.syncStatus === 'error').length;
        ...
        console.warn('⚠️ [OfflineManager] Error accediendo a IndexedDB para estadísticas:', dbError);
      }
      ...
      console.error('❌ [OfflineManager] Error obteniendo estadísticas:', error);
   ...
   */
  private deduplicateVisitas(indexedDBVisitas: any[], fallbackVisitas: any[]): {
    uniqueIndexedDB: any[];
    uniqueFallback: any[];
    ...
    for (const fallbackVisita of fallbackVisitas) {
      ...
      if (indexedDBHashes.has(fallbackHash)) {
        console.log(`🗑️ [OfflineManager] Duplicado encontrado y eliminado: ${fallbackHash.substring(0, 8)}...`);
        duplicatesRemoved++;
    ...
    console.log(`✅ [OfflineManager] Deduplicación completada: ${duplicatesRemoved} duplicados eliminados`);
  ...
  private isAdminData(data: any): boolean {
    return data.tipoVisita?.includes('Admin -') || 
           data.accion || 
           data.clienteData || 
           data.userData || 
           data.routeData || 
  ...
  private async syncAdminData(data: any): Promise<void> {
    console.log('🔄 [OfflineManager] Sincronizando datos administrativos:', data.tipoVisita);
    
    const { getFirestoreClient } = await import('@/firebase/clientApp');
    const { collection, addDoc, updateDoc, doc, setDoc, query, where, getDocs } = await import('firebase/firestore');
      ...
      if (data.tipoVisita?.includes('Admin - Gestión Cliente')) {
        ...
        if (data.accion === 'crear') {
          ...
          if (existingClients.empty) {
            await addDoc(collection(getFirestoreClient(), 'clientes'), data.clienteData);
            console.log('✅ [OfflineManager] Cliente creado (no existía duplicado)');
          } else {
            console.log('⚠️ [OfflineManager] Cliente ya existe, omitiendo creación duplicada');
          }
        } else if (data.accion === 'actualizar' && data.clienteId) {
          await updateDoc(doc(getFirestoreClient(), 'clientes', data.clienteId), {
            ...data.clienteData,
            updatedAt: new Date()
          });
        }
        ...
        // Sincronizar ruta
        await addDoc(collection(getFirestoreClient(), 'routes'), data.routeData);
        console.log('✅ [OfflineManager] Ruta sincronizada');
        
      } else if (data.tipoVisita?.includes('Admin - Gestión Evento')) {
        // Sincronizar evento
        if (data.accion === 'crear') {
          await addDoc(collection(getFirestoreClient(), 'eventos'), data.eventoData);
        } else if (data.accion === 'actualizar' && data.eventoId) {
          await updateDoc(doc(getFirestoreClient(), 'eventos', data.eventoId), data.eventoData);
        }
        ...
        if (data.accion === 'crear') {
          ...
          if (existingUsers.empty) {
            await setDoc(doc(getFirestoreClient(), 'users', data.userId), data.userData);
            console.log('✅ [OfflineManager] Usuario creado (no existía duplicado)');
          } else {
            console.log('⚠️ [OfflineManager] Usuario ya existe, omitiendo creación duplicada');
          }
        } else if (data.accion === 'actualizar' && data.userId) {
          await setDoc(doc(getFirestoreClient(), 'users', data.userId), data.userData, { merge: true });
          console.log('✅ [OfflineManager] Usuario actualizado');
        }
        
      } else {
        console.warn('⚠️ [OfflineManager] Tipo de dato administrativo no reconocido:', data.tipoVisita);
      }
    } catch (error) {
      console.error('❌ [OfflineManager] Error sincronizando datos administrativos:', error);
      throw error;
    }
  }

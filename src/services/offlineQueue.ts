...
import { db as fs } from '@/firebase/clientApp';
...
export async function queueOperation(op: Omit<PendingOperation, 'id' | 'status' | 'retries' | 'maxRetries' | 'createdAt' | 'updatedAt'>) {
  const now = Date.now();
...
export async function queueUploadImage(params: { draftId: string; fieldKey: string; base64: string; storagePath: string }) {
...
export async function queueCreateVisita(params: { draftId: string; collection: string; data: any }) {
...
export async function queueWebhookN8N(params: { draftId: string; url: string; body: any }) {
...
export async function processQueue(): Promise<{ processed: number; errors: number }> {
  ...
  for (const op of pending) {
    try {
      await db.pendingOps.update(op.id, { status: 'processing', updatedAt: Date.now() });
      switch (op.type as PendingType) {
        case 'uploadImage': {
          ...
          await db.pendingOps.update(op.id, { status: 'completed', updatedAt: Date.now() });
          processed++;
          break;
        }
        case 'createVisita': {
        ...
        default: {
          await db.pendingOps.update(op.id, { status: 'failed', updatedAt: Date.now(), lastError: 'Tipo no soportado' });
          errors++;
        }
      }
    } catch (e: any) {

import { doc, updateDoc, Timestamp, onSnapshot, collection, query, where, getDocs, QuerySnapshot, DocumentData } from 'firebase/firestore';
    ...
    querySnapshot.forEach(doc => {
      ...
      if (userData.role === 'AdminMaster' || userSede === sede) {
        if (userData.email && userData.fullName) {
          ...
          console.log(`✅ Admin encontrado: ${userData.fullName} (${userData.email}) - Sede: ${userSede}`);
        }
      }
    });
    
    console.log(`📊 Total administradores encontrados para ${sede}: ${admins.length}`);
    return admins;
...
export const updateRouteStatus = async (
  ...
  try {
    console.log(`🔄 [RouteService] Cambiando status de ruta ${routeId} a: ${newStatus}`);
    ...
    throw new Error(`No se pudo cambiar el estado de la ruta: ${error?.message || 'Error desconocido'}`);
  }
};
...
export const updateRoutePointStatus = async (
  ...
  try {
    console.log(`🎯 [RouteService] === INICIANDO ACTUALIZACIÓN DE PUNTO ESPECÍFICO ===`);
    ...
    if (snapshot.empty) {
      console.log('⚠️ [RouteService] No se encontraron rutas para la fecha específica');
      return { updated: false, reason: `No se encontraron rutas para la fecha ${date}` };
    }

    console.log(`🎯 [RouteService] Encontradas ${snapshot.docs.length} rutas SOLO para la fecha ${date}`);
    ...
    for (const routeDoc of snapshot.docs) {
      ...
      console.log(`🎯 [RouteService] === ANALIZANDO RUTA ${routeDoc.id} (Fecha: ${routeData.date}) ===`);
      ...
      if (pointId) {
        pointIndex = points.findIndex((point: RoutePoint) => point.id === pointId);
        console.log(`🎯 [RouteService] Búsqueda EXACTA por pointId "${pointId}": ${pointIndex >= 0 ? `ENCONTRADO en índice ${pointIndex}` : 'NO ENCONTRADO'}`);
        
        if (pointIndex >= 0) {
          console.log(`🎯 [RouteService] ✅ PUNTO ENCONTRADO EXACTAMENTE: "${points[pointIndex].name}" con estado actual "${points[pointIndex].status}"`);
          routeFound = true;
          ...
          if (points[pointIndex].status !== newStatus) {
            ...
            console.log(`✅ [RouteService] Punto actualizado exitosamente: "${points[pointIndex].name}" → "${newStatus}"`);
            pointUpdated = true;
          } else {
            console.log(`ℹ️ [RouteService] Punto ya tiene el estado "${newStatus}", no se requiere actualización`);
      ...
      console.log(`🎯 [RouteService] No se usará búsqueda por RIF para evitar confusiones entre rutas`);
    }

    if (!routeFound) {
      console.log(`⚠️ [RouteService] === PUNTO NO ENCONTRADO EN LA FECHA ESPECÍFICA ===`);
      console.log(`⚠️ [RouteService] No se encontró el punto con ID: "${pointId}" en rutas del ${date}`);
      return { updated: false, reason: `Punto no encontrado en las rutas del ${date}` };
    }

    const result = { 
      updated: pointUpdated, 
      reason: pointUpdated ? `Punto actualizado exitosamente para la fecha ${date}` : `No se pudo actualizar el punto para la fecha ${date}`
    };
    ...
    throw new Error(`No se pudo actualizar el punto de ruta: ${error?.message || 'Error desconocido'}`);
  }
};
 ...
 */
export const startRoute = async (routeId: string, mercaderistoId: string): Promise<void> => {
  console.log(`🚀 [RouteService] Iniciando ruta ${routeId} para mercaderista ${mercaderistoId}`);
 ...
 */
export const completeRoute = async (routeId: string, mercaderistoId: string): Promise<void> => {
  console.log(`🏁 [RouteService] Completando ruta ${routeId} para mercaderista ${mercaderistoId}`);
...
export const listenToMercaderistaRoutes = (
  ...
  console.log(`👂 [RouteService] Configurando listener para mercaderista ${mercaderistoId} en fecha ${date}`);
  ...
  const unsubscribe = onSnapshot(
    ...
    (snapshot) => {
      ...
      console.log(`📊 [RouteService] ${routes.length} rutas actualizadas para ${mercaderistoId}`);
      callback(routes);
    },
    (error) => {
...
export const autoUpdateRouteStatus = async (
  ...
  try {
      ...
      if (action === 'start' && currentStatus === 'planificada') {
        ...
        if (currentStatus === 'en_progreso') {
          ...
          try {
            ...
            console.log(`👤 Mercaderista: ${route.mercaderista} (ID: ${mercaderistoId})`);
            ...
            if (!route.sede) {
              console.log('⚠️ Ruta sin sede asignada, usando sede por defecto:', sede);
            }
            ...
            console.log(`📊 Administradores encontrados: ${adminEmails.length}`);
            
            if (adminEmails.length > 0) {
              console.log(`📧 Preparando envío a ${adminEmails.length} administradores:`);
              adminEmails.forEach(admin => {
                console.log(`   👤 ${admin.fullName} (${admin.email}) - Sede: ${admin.sede}`);
              });
              ...
              const emailPromises = adminEmails.map(admin => {
                ...
                console.log(`📤 Enviando email a: ${admin.fullName} (${admin.email})`);
              ...
              results.forEach((result, index) => {
                ...
                if (result.status === 'fulfilled') {
                  if (result.value === true) {
                    console.log(`✅ Email enviado exitosamente a: ${admin.fullName}`);
                  } else {
                    console.log(`❌ Error enviando email a: ${admin.fullName}`);
                  }
                } else {
                  console.log(`❌ Promise rechazado para: ${admin.fullName}`, result.reason);
                }
              });
              ...
              if (successCount > 0) {
                console.log(`🎉 RESUMEN: Email de ruta completada enviado a ${successCount}/${adminEmails.length} administradores`);
              } else {
                console.log('💥 PROBLEMA: No se pudo enviar email a ningún administrador');
                console.log('🔍 Revisar logs anteriores para más detalles');
              }
            } else {
              console.log('💥 PROBLEMA: No se encontraron administradores para enviar email');
              console.log('💡 POSIBLES CAUSAS:');
              console.log('   1. No hay administradores en la base de datos');
              console.log('   2. Los administradores no tienen emails válidos');
              console.log('   3. Los administradores no pertenecen a la sede:', sede);
            }
          } catch (notificationError) {
            console.error('❌ Error enviando notificación de ruta completada:', notificationError);
    ...
    throw new Error(`No se pudo actualizar automáticamente: ${error?.message || 'Error desconocido'}`);
  }
};
...
export const updateRoutePointsFromExistingVisits = async (
  ...
  try {
    console.log(`🔄 [Migration] Actualizando puntos de ruta retroactivamente para mercaderista: ${mercaderistoId}, fecha: ${date}`);
    ...
    if (routesSnapshot.empty) {
      ...
      return { updated: false, pointsUpdated: 0, reason: 'No se encontraron rutas para la fecha' };
    }
    ...
    console.log(`📊 [Migration] Encontradas ${visitasDelDia.length} visitas para el día ${date}`);
    ...
    for (const routeDoc of routesSnapshot.docs) {
      ...
      console.log(`🔄 [Migration] Procesando ruta ${routeDoc.id} con ${points.length} puntos`);
      ...
      const updatedPoints = points.map((point: RoutePoint) => {
        ...
        const visitaDelPunto = visitasDelDia.find((visita: any) => {
          if (point.rif && visita.rifCliente) {
            return point.rif.trim().toUpperCase() === visita.rifCliente.trim().toUpperCase();
          }
          return false;
        });

        if (visitaDelPunto && point.status === 'pendiente') {
          console.log(`✅ [Migration] Actualizando punto ${point.name} de pendiente a visitado`);
          routePointsUpdated++;
      ...
      if (routePointsUpdated > 0) {
        ...
        console.log(`✅ [Migration] Ruta ${routeDoc.id} actualizada con ${routePointsUpdated} puntos`);
      }
    }
    ...
    throw new Error(`Error en migración: ${error?.message || 'Error desconocido'}`);
  }
};
...
export const updateEventStatus = async (
  ...
  try {
    console.log(`🎪 [EventService] Cambiando status de evento ${eventId} a: ${newStatus}`);
    ...
    console.log(`✅ [EventService] Status de evento actualizado a: ${newStatus}`);
  } catch (error: any) {
    console.error('❌ [EventService] Error actualizando status del evento:', error);
    throw new Error(`No se pudo cambiar el estado del evento: ${error?.message || 'Error desconocido'}`);
  }
};

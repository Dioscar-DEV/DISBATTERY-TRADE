...
export const crearVisita = async (data: CreateVisitaData): Promise<string> => {
  try {
    ...
    console.log(
      "✅ Nombre guardado en Firestore:",
      visitaData.nombreEstablecimiento
    );
    ...
    if (data.rifCliente) {
      ...
      const clientesQuery = query(
        ...
        where("rif", "==", rifNormalizado)
      );
      const querySnapshot = await getDocs(clientesQuery);
      if (!querySnapshot.empty) {
        const clienteDoc = querySnapshot.docs[0];
        const clienteRef = doc(getFirestoreClient(), "clientes", clienteDoc.id);
        ...
        console.log(
          "🚩 data.respuestas:",
          JSON.stringify(data.respuestas, null, 2)
        );
        console.log(
          "🚩 data.respuestas.clientePoseeSeñalizacion:",
          (data.respuestas as any)?.clientePoseeSeñalizacion
        );
        console.log(
          "🚩 data.respuestas.fotoSeñalizacion:",
          (data.respuestas as any)?.fotoSeñalizacion
        );
        console.log(
          "🚩 data.respuestas.datosSheet:",
          JSON.stringify((data.respuestas as any)?.datosSheet, null, 2)
        );
        ...
        try {
          ...
          const directos = [
            {
              key: "clientePoseeSeñalizacion",
              value: resp.clientePoseeSeñalizacion,
            },
            ...
            {
              key: 'ds["¿El cliente posee señalización?"]',
              value: ds?.["¿El cliente posee señalización?"],
            },
          ];
          ...
          for (const item of directos) {
            ...
            if (
              ...
              const esNegativo =
                ...
                s === "0";
              console.log(
                `     🎯 DETECTADO EN ${item.key}: valor="${item.value}", esPositivo=${esPositivo}, esNegativo=${esNegativo}`
              );
              if (esPositivo) {
          ...
          const fuentesFoto = [
            ...
            {
              key: 'ds["Foto de la señalización"]',
              value: ds?.["Foto de la señalización"],
            },
            {
              key: 'ds["Foto señalización"]',
              value: ds?.["Foto señalización"],
            },
            {
              key: 'ds["Foto de señalizacion"]',
              value: ds?.["Foto de señalizacion"],
            },
          ...
          for (const fuente of fuentesFoto) {
            console.log(
              `   - ${fuente.key}:`,
              fuente.value ? "ENCONTRADA" : "NO ENCONTRADA"
            );
            if (
              fuente.value &&
              typeof fuente.value === "string" &&
              fuente.value.trim() !== "" &&
              fuente.value !== "No capturada"
            ) {
          ...
          if (tieneSenalizacion === undefined && ds && typeof ds === "object") {
            ...
            if (/señaliz|signage|letrero|cartel/.test(texto)) {
              if (
                /\b(sí|si|yes|true|presente|instalada|colocada)\b/.test(texto)
              )
                tieneSenalizacion = true;
              if (/\b(no|false|ausente|falta|sin|missing)\b/.test(texto))
                tieneSenalizacion = false;
            }
          }
        } catch (e) {
          console.warn(
            "⚠️ No se pudo derivar señalización desde respuestas:",
            e
          );
        }
        ...
        const updatePayload: any = {
          ...
          updatedAt: new Date(),
          updatedBy:
            data.correoMercaderista || data.mercaderista || "mercaderista",
        };
        ...
        if (
          tieneSenalizacion === undefined &&
          fotoSenalizacion &&
          fotoSenalizacion.trim() !== "" &&
          fotoSenalizacion !== "No capturada"
        ) {
          console.log(
            "🚩 NO SE DETECTÓ SEÑALIZACIÓN DIRECTA, PERO HAY FOTO VÁLIDA - ASUMIENDO SEÑALIZACIÓN POSITIVA"
          );
          tieneSenalizacion = true;
        }
        ...
        if (tieneSenalizacion !== undefined) {
          ...
          console.log(
            "🚩 ACTUALIZANDO SIGNAGE EN CLIENTE:",
            updatePayload.signage
          );
          ...
          console.log(
            "🚩 NO SE DETECTÓ SEÑALIZACIÓN NI FOTO - NO SE ACTUALIZA"
          );
          console.log(
            "🚩 VALORES FINALES: tieneSenalizacion =",
            tieneSenalizacion,
            ", fotoSenalizacion =",
            fotoSenalizacion ? "PRESENTE" : "AUSENTE"
          );
        }
        ...
        try {
          const lat = Number(
            (data.ubicacion as any)?.lat ??
              (data.ubicacion as any)?.latitude ??
              0
          );
          const lng = Number(
            (data.ubicacion as any)?.lng ??
              (data.ubicacion as any)?.longitude ??
              0
          );
          if (
            Number.isFinite(lat) &&
        ...
        console.warn(
          ...
          rifNormalizado,
          "para actualizar lastVisitDate/señalización"
        );
      }
    }
    ...
    if (data.rifCliente && data.correoMercaderista) {
      try {
        ...
        if (clienteDataString) {
          ...
          console.log(
            "🎯 [crearVisita] PointId obtenido desde localStorage:",
            pointId
          );
          console.log("🎯 [crearVisita] ClienteData completo:", clienteData);
          console.log(
            "🎯 [crearVisita] RIF del cliente desde localStorage:",
            clienteData.rif
          );
          console.log(
            "🎯 [crearVisita] Nombre del cliente desde localStorage:",
            clienteData.nombre
          );
        } else {
          console.warn(
            "⚠️ [crearVisita] No se encontró clienteData en localStorage"
          );
        }
        ...
        if (!pointId) {
          console.warn(
            "⚠️ [crearVisita] PointId vacío en localStorage, buscando en base de datos..."
          );
          ...
          try {
            ...
            console.log(
              "🔍 [crearVisita] Buscando pointId en",
            ...
            for (const routeDoc of snapshot.docs) {
              ...
              if (matchingPoint && matchingPoint.id) {
                ...
                console.log(
                  "✅ [crearVisita] PointId encontrado en base de datos:",
                  pointId
                );
                console.log(
                  "✅ [crearVisita] Punto encontrado:",
                  matchingPoint.name
                );
                break;
              }
            ...
            if (!pointId) {
              console.warn(
                "⚠️ [crearVisita] PointId no encontrado en base de datos para RIF:",
                rifNormalizado
              );
            }
          } catch (error) {
            console.error(
              "❌ [crearVisita] Error buscando pointId en base de datos:",
              error
            );
          }
        }
        ...
        if (!pointId) {
          console.warn(
            "⚠️ [crearVisita] ADVERTENCIA: pointId está vacío, se usará solo RIF como fallback"
          );
        } else {
        ...
        if (mercaderistoId) {
          ...
          console.log(
            "🎯 [crearVisita] === PREPARANDO ACTUALIZACIÓN DE PUNTO ==="
          );
          ...
          console.log(
            "🎯 [crearVisita] RIF Cliente normalizado:",
            rifNormalizado
          );
          ...
          if (result.updated) {
            console.log(
              "✅ [crearVisita] Estado del punto de ruta actualizado exitosamente:",
              result.reason
            );
            // ✅ Asegurar que la ruta quede en "en_progreso" al registrar la primera visita
            try {
              const startRes = await autoUpdateRouteStatus(
                mercaderistoId,
                today,
                "start"
              );
              console.log(
                "🚀 [crearVisita] Auto-start de ruta tras visita:",
                startRes
              );
            } catch (e) {
              console.warn(
                "⚠️ [crearVisita] No se pudo auto-iniciar la ruta:",
                e
              );
            }
          } else {
            console.log(
              "ℹ️ [crearVisita] No se actualizó punto de ruta:",
              result.reason
            );
          }
        } else {
          console.warn(
            "⚠️ [crearVisita] No se pudo obtener mercaderistoId para actualizar punto de ruta"
          );
        }
      } catch (error) {
    ...
    try {
      ...
      if (esEvento) {
        console.log(
          "🎪 [crearVisita] ✅ EVENTO DETECTADO - Actualizando estado..."
        );
        ...
        const eventosQuery = query(
          eventosRef,
          where("nombreEvento", "==", data.nombreEstablecimiento)
        );
        ...
        if (!eventosSnapshot.empty) {
          ...
          await updateDoc(doc(getFirestoreClient(), "eventos", eventoId), {
            status: "completado",
          ...
          console.warn(
            "⚠️ [crearVisita] No se encontró evento:",
            data.nombreEstablecimiento
          );
        }
      }
    ...
    await enviarANBN(visitaCompleta, data.datosN8N).catch((error) => {
      ...
      updateDoc(doc(getFirestoreClient(), "visitas", docRef.id), {
        sincronizadoN8N: false,
...
const enviarANBN = async (visita: any, datosN8N?: any): Promise<void> => {
  ...
  try {
    ...
    if (datosN8N && datosN8N.datosSheet) {
      ...
      console.log(
        "🎯 USANDO DATOS COMPLETOS DE N8N PASADOS DIRECTAMENTE (CON IMÁGENES):",
        Object.keys(datosSheet).length,
        "campos"
      );
    }
    // Si no hay datos N8N, usar las respuestas normales
      ...
      if (respuestas.datosSheet && typeof respuestas.datosSheet === "object") {
        ...
        console.log(
          "✅ USANDO ESTRUCTURA ORGANIZADA NUEVA:",
          Object.keys(datosSheet).length,
          "campos"
        );
      }
      // PRIORIDAD 2: Convertir observaciones organizadas a datosSheet
        ...
        console.log(
          "✅ USANDO OBSERVACIONES ORGANIZADAS:",
          Object.keys(datosSheet).length,
          "campos"
        );
      } else {
        ...
        console.log(
          "⚠️ USANDO ESTRUCTURA ANTIGUA (fallback):",
          Object.keys(datosSheet).length,
          "campos"
        );
      }
      ...
      datosSheet = mapearParaSheet(visita);
      console.log(
        "⚠️ USANDO ESTRUCTURA ANTIGUA (sin respuestas):",
        Object.keys(datosSheet).length,
        "campos"
      );
    }
    ...
    const payload = {
      ...
      datosSheet: {
        ...
        "Latitud:":
          visita.ubicacion?.latitude || visita.ubicacion?.lat || "No capturada",
        "Longitud:":
          visita.ubicacion?.longitude ||
          visita.ubicacion?.lng ||
          "No capturada",
        "Dirección GPS:":
          visita.ubicacion?.address ||
          visita.ubicacion?.direccion ||
          "No disponible",
        "Coordenadas completas:": visita.ubicacion
          ? JSON.stringify(visita.ubicacion)
          : "No disponibles",
        "Estado de coordenadas:":
          visita.ubicacion?.lat === 0 && visita.ubicacion?.lng === 0
            ? "No capturadas correctamente"
            : "Capturadas correctamente",
    ...
    console.log(
      "- Campos en datosSheet:",
      Object.keys(payload.datosSheet).length
    );
    console.log(
      "- RIF en datosSheet:",
      (payload.datosSheet as any)["Rif del cliente:"]
    );
    console.log(
      "- Nombre en datosSheet:",
      (payload.datosSheet as any)["Nombre del establecimiento:"]
    );
    console.log("🗺️ COORDENADAS EN DATOSSHEET:");
    console.log(
      "- Latitud en datosSheet:",
      (payload.datosSheet as any)["Latitud:"]
    );
    console.log(
      "- Longitud en datosSheet:",
      (payload.datosSheet as any)["Longitud:"]
    );
    console.log(
      "- Dirección GPS en datosSheet:",
      (payload.datosSheet as any)["Dirección GPS:"]
    );
    ...
    await updateDoc(doc(getFirestoreClient(), "visitas", visita.id), {
    ...
    const tieneFotos =
      payload.datosSheet &&
      Object.values(payload.datosSheet).some(
        (valor: any) =>
          typeof valor === "string" && valor.startsWith("data:image/")
      );

    console.log(
      "🎉 DATOS ENVIADOS EXITOSAMENTE A N8N CON ESTRUCTURA ORGANIZADA"
    );
    console.log(
      "📊 Campos enviados:",
      Object.keys(payload.datosSheet).slice(0, 10),
      "... (total:",
      Object.keys(payload.datosSheet).length,
      ")"
    );
    console.log(
      `📸 Fotos incluidas en el envío: ${tieneFotos ? "✅ SÍ" : "❌ NO"}`
    );

    if (tieneFotos) {
      const fotosCount = Object.values(payload.datosSheet).filter(
        (valor: any) =>
          typeof valor === "string" && valor.startsWith("data:image/")
      ).length;
...
export const reintentarSincronizacion = async (
  visitaId: string
): Promise<void> => {
  try {
...
export const obtenerUltimaVisitaUsuario = async (
...
export const obtenerUltimasVisitasUsuarios = async (
  correosMercaderistas: string[]
): Promise<Record<string, Visita | null>> => {
